...
\title[Processi stocastici nel tempo discreto]{Lezione 5 -- Processi stocastici I - Tempo Discreto}
...
% --------------------------------------------------------
% Slide 4 -- Tre livelli di lettura
% --------------------------------------------------------
...
% --------------------------------------------------------
% Slide 5 -- Esempio finanziario: prezzo su più periodi
% --------------------------------------------------------
...
% --------------------------------------------------------
% Slide 6 -- Spazio campionario e traiettorie
% --------------------------------------------------------
...
% --------------------------------------------------------
% Slide 7 -- Le tre funzioni caratteristiche
% --------------------------------------------------------
...
% --------------------------------------------------------
% Slide 8 -- Significato finanziario
% --------------------------------------------------------
...
% --------------------------------------------------------
% Slide 9 -- Esempio 1: rumore i.i.d.
% --------------------------------------------------------
...
% --------------------------------------------------------
% Slide 10 -- Esempio 2: passeggiata aleatoria
% --------------------------------------------------------
...
% --------------------------------------------------------
% Slide 11 -- Confronto e ponte al moto browniano
% --------------------------------------------------------
...
% --------------------------------------------------------
% Slide 12 -- Stazionariet\`{a} forte e debole
% --------------------------------------------------------

\subsection{Stazionariet\`{a} forte e debole}
...
\QTR{frametitle}{Stazionariet\`{a} forte e debole}
...
La stazionariet\`{a} formalizza l'idea che la struttura probabilistica
del processo non cambi quando si sposta l'origine del tempo.

\medskip

\begin{stepitemize}

\item \textbf{Stazionariet\`{a} forte}: per ogni scelta di istanti
$t_1,\ldots,t_n$ e per ogni traslazione ammissibile $h$,
\[
(X_{t_1+h},\ldots,X_{t_n+h})
\quad \hbox{ha la stessa legge di} \quad
(X_{t_1},\ldots,X_{t_n}).
\]
Riguarda l'intera distribuzione congiunta del processo.

\item \textbf{Stazionariet\`{a} debole}: se esistono i momenti secondi,
\[
\E[X_t]=\mu, \qquad
\Cov(X_s,X_t)=\gamma(t-s).
\]
Media costante e autocovarianza dipendente solo dalla distanza temporale.

\item La stazionariet\`{a} forte implica quella debole, se i momenti
secondi esistono; il viceversa non vale in generale.
...
% --------------------------------------------------------
% Slide 13 -- Incrementi indipendenti e stazionari
% --------------------------------------------------------

\subsection{Incrementi indipendenti e stazionari}
...
\QTR{frametitle}{Incrementi indipendenti e incrementi stazionari}
...
Per un processo a tempo discreto, l'incremento elementare \`{e}
\[
...
\item \textbf{Incrementi indipendenti}: le variazioni osservate su
intervalli temporali disgiunti sono variabili casuali indipendenti.
In particolare, gli shock futuri non dipendono dagli shock passati.

\item \textbf{Incrementi stazionari}: la distribuzione di
\[
X_{t+h}-X_t
\]
dipende solo dall'ampiezza $h$ dell'intervallo, non dalla posizione
temporale $t$.

\item Interpretazione finanziaria: gli incrementi descrivono le
\textbf{innovazioni} del processo. Un modello pu\`{o} avere innovazioni
stabili nel tempo anche se il livello del processo non \`{e} stazionario.
...
% --------------------------------------------------------
% Slide 14 -- Processo e incrementi: propriet\`{a} distinte
% --------------------------------------------------------

\subsection{Propriet\`{a} distinte}
...
\QTR{frametitle}{Processo stazionario e incrementi stazionari}
...
Non bisogna confondere la stazionariet\`{a} del processo con la
stazionariet\`{a} dei suoi incrementi.
...
\begin{tabular}{lcc}
\hline
 & \textbf{Processo stazionario} & \textbf{Incrementi stazionari}\\
\hline
$R_t=Z_t$ i.i.d. & s\`{i} & --\\
$X_t=\sum_{k=1}^t Z_k$ & no & s\`{i}\\
\hline
\end{tabular}
}
\end{center}
...
\item I rendimenti i.i.d. hanno distribuzione stabile nel tempo:
il processo non accumula livello.

\item La passeggiata aleatoria ha invece
\[
\Var(X_t)=t\,\tau^2,
\]
quindi non \`{e} stazionaria: l'incertezza cresce con l'orizzonte.

\item Tuttavia i suoi incrementi sono proprio gli shock $Z_t$:
se gli $Z_t$ sono i.i.d., gli incrementi sono indipendenti e stazionari.
...
% --------------------------------------------------------
% Slide 15 -- Il problema: informazione che cresce nel tempo
% --------------------------------------------------------
...
% --------------------------------------------------------
% Slide 16 -- Definizione di filtrazione
% --------------------------------------------------------
...
% --------------------------------------------------------
% Slide 18 -- Processo adattato e anticipativo
% --------------------------------------------------------
...
% --------------------------------------------------------
% Slide 19 -- Esempi: adattato vs anticipativo
% --------------------------------------------------------
...
% --------------------------------------------------------
% Slide 20 -- Implicazione finanziaria
% --------------------------------------------------------
...
% --------------------------------------------------------
% Slide 21 -- Tabella sinottica prima parte
% --------------------------------------------------------
...
% --------------------------------------------------------
% Slide 23 -- Conseguenze e sub/supermartingala
% --------------------------------------------------------
...
% --------------------------------------------------------
% Slide 24 -- Esempio: profitto cumulato
% --------------------------------------------------------
...
% --------------------------------------------------------
% Slide 26 -- Trasformata di martingala e arresto opzionale
% --------------------------------------------------------
...
% --------------------------------------------------------
% Slide 27 -- Cambio di misura e pricing
% --------------------------------------------------------
...
% --------------------------------------------------------
% Slide 28 -- Definizione del modello
% --------------------------------------------------------
...
\item \textbf{Varianza}: introdotto il momento secondo del
fattore di rendimento $m_2=\E[R_t^2]=pu^2+(1-p)d^2$, si ha
$\E[S_t^2]=S_0^2\,m_2^t$, da cui
\[
...
% --------------------------------------------------------
% Slide 31 -- Ricombinazione: concetto
% --------------------------------------------------------
...
% --------------------------------------------------------
% Slide 32 -- Sigma-algebre e partizioni sull'albero
% --------------------------------------------------------
...
% --------------------------------------------------------
% Slide 33 -- Processo delle previsioni sull'albero
% --------------------------------------------------------
...
% --------------------------------------------------------
% Slide 34 -- Martingala di Doob e prezzi
% --------------------------------------------------------
...
% --------------------------------------------------------
% Slide 35 -- Sintesi del ruolo del binomiale
% --------------------------------------------------------
...
% --------------------------------------------------------
% Slide 37 -- Esercizio 1: soluzione 1/2 (albero)
% --------------------------------------------------------
...
\QTR{frametitle}{Esercizio 1: soluzione (1/2) -- l'albero}
...
Il rendimento lordo medio per periodo \`{e}
$m=pu+(1-p)d=0.5\cdot1.25+0.5\cdot0.8=1.025$.
...
\begin{stepitemize}
\item Quattro scenari, ciascuno con probabilit\`{a} $0.25$
(essendo $p=0.5$): $uu\to125$; $ud$ e $du\to80$; $dd\to51.2$.
\item L'albero ricombina: al tempo $t=2$ i nodi distinti sono
$3$, le traiettorie $4$.
\end{stepitemize}
...
% --------------------------------------------------------
% Slide 38 -- Esercizio 1: soluzione 2/2 (attese e torre)
% --------------------------------------------------------
...
\QTR{frametitle}{Esercizio 1: soluzione (2/2) -- valori attesi e torre}
...
\item \textbf{Valore atteso terminale}, in due modi:
\[
\E[S_2]=0.25\cdot125+0.5\cdot80+0.25\cdot51.2=84.05,
\]
in accordo con la formula compatta
$S_0\,m^2=80\cdot1.025^2=84.05$. Le due strade devono
coincidere. 
\item \textbf{Valore atteso condizionato}: $\E[S_2\mid\F_1]$
\`{e} una \emph{variabile casuale}, con un valore per ciascun
nodo al tempo $1$:
\[
\E[S_2\mid A_u]=0.5\cdot125+0.5\cdot80=102.5=S_1^u\cdot m,
\]
\[
\E[S_2\mid A_d]=0.5\cdot80+0.5\cdot51.2=65.6=S_1^d\cdot m.
\]
In ogni nodo, la previsione \`{e} il prezzo corrente
capitalizzato al rendimento medio.
 
\item \textbf{Propriet\`{a} della torre}: mediando le
previsioni con le probabilit\`{a} dei nodi,
\[
0.5\cdot102.5+0.5\cdot65.6=84.05=\E[S_2]. \;\checkmark
\]
La previsione della previsione coincide con la previsione.
...
% --------------------------------------------------------
% Slide 39 -- Esercizio 2: interpretazione
% --------------------------------------------------------
...
% --------------------------------------------------------
% Slide 40 -- Esercizio 2: soluzione
% --------------------------------------------------------
...
\section{Sintesi conclusiva}

% --------------------------------------------------------
% Slide 41 -- Sintesi e collegamenti successivi
% --------------------------------------------------------
...
\QTR{frametitle}{Sintesi: processi, informazione e previsioni}
...
La Lezione 5 ha introdotto il linguaggio di base per modellare
l'incertezza che evolve nel tempo.

\medskip

\begin{stepitemize}

\item \textbf{Processo stocastico}: una famiglia
$\{X_t\}_{t=0}^{T}$ descrive distribuzioni marginali, traiettorie
e struttura congiunta nel tempo.

\item \textbf{Dipendenza temporale}: media, varianza, autocovarianza,
stazionariet\`{a} e incrementi descrivono come l'incertezza si
propaga lungo l'orizzonte.

\item \textbf{Informazione}: la filtrazione
\[
\F_0\subseteq\F_1\subseteq\cdots\subseteq\F_T
\]
rappresenta ci\`{o} che \`{e} noto progressivamente.

\item \textbf{Martingala}: la condizione
\[
\E[X_{t+1}\mid\F_t]=X_t
\]
formalizza l'idea di previsione equa rispetto all'informazione disponibile.

\item Questi strumenti saranno riutilizzati nei processi in tempo continuo,
nelle catene di Markov, nella simulazione per scenari e nei vincoli
di non anticipativit\`{a} della programmazione stocastica.
