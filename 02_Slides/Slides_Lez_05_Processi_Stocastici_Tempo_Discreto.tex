...
Sia $X_0 = 0$ e $X_t = Z_1 + Z_2 + \cdots + Z_t$
con $Z_k$ i.i.d., $\E[Z_k] = \nu$, $\Var(Z_k) = \tau^2$.
...
chi parte alto tende a restare alto.
...
\QTR{frametitle}{Confronto e ponte alla Lezione 6}
...
quadruplica l'incertezza sul valore finale... no: la raddoppia.
...
\]
la pi\`{u} piccola sigma-algebra che rende misurabili tutti
...
\]
\item \textbf{Anche il quadrato porta informazione}: il processo
\[
X_t^2 - \Var(X_t) = X_t^2 - t\tau^2 \quad (\text{caso } \nu=0)
\]
\`{e} una martingala. \`{E} il primo esempio di \emph{variazione
quadratica}, nozione centrale nel passaggio al tempo continuo
(Lezione 6).
\end{stepitemize}
...
\begin{center}
\includegraphics[width=0.85\textwidth]{../graphics/Cap04_Figura_4_1}
\end{center}
