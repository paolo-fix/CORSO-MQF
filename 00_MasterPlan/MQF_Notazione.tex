...
\verb|\mathbf{1}|. Questa notazione \`{e} preferita a $\mathbb{I_A}$ e a
...
\]
Nel caso continuo, la funzione di ripartizione condizionata di \(X\)
dato \(A\), con \(\Prob(A)>0\), \`{e}
\[
...
Sia \((\Omega,\mathcal{F},\mathbb{P})\) uno spazio di probabilit\`{a}
e sia \(\mathcal{G}\subseteq\mathcal{F}\) una sigma-algebra informativa.
Per variabili casuali integrabili, valgono le seguenti regole.
...
In particolare, se \(Z\) \`{e} noto rispetto all'informazione
\(\mathcal{G}\), pu\`{o} essere estratto dal valore atteso condizionato.

\paragraph{Condizionamento iterato.}
Se \(\mathcal{G}_1\subseteq\mathcal{G}_2\subseteq\mathcal{F}\), allora:
\[
\E\left[
\E[X\mid\mathcal{G}_2]
\mid
\mathcal{G}_1
\right]
=
...
Se \(\varphi:\mathbb{R}\to\mathbb{R}\) \`{e} convessa e
\(X\), \(\varphi(X)\) sono integrabili, allora:
\[
...
Il corso introduce i processi in tempo continuo a scopo motivazionale e
concettuale, mantenendo il baricentro applicativo sul tempo discreto.
I simboli di questa sezione vengono utilizzati principalmente nel Capitolo~4
e nei riferimenti ai modelli di pricing del Capitolo~9.

\begin{longtable}{>{\raggedright}p{3.4cm}>{\raggedright}p{4.4cm}>{\raggedright\arraybackslash}p{6.6cm}}
\hline
\textbf{Concetto} & \textbf{Notazione} & \textbf{Significato}\\
\hline
Moto browniano standard & $\{W_t\}_{t\geq 0}$ &
Processo con $W_0=0$, incrementi indipendenti e stazionari,
$W_t-W_s\sim\mathcal{N}(0,t-s)$.\\
Incremento browniano & $dW_t$ &
Incremento infinitesimo del moto browniano, usato nelle SDE.\\
Moto browniano geometrico & $\{S_t\}_{t\geq 0}$ &
Processo di prezzo con $S_t=S_0\exp(\mu t+\sigma W_t)$.\\
Drift & $\mu$ &
Parametro di tendenza media in un processo diffusivo.\\
Volatilit\`{a} diffusiva & $\sigma$ &
Parametro di dispersione in un processo diffusivo; $\sigma>0$.\\
Equazione differenziale stocastica & $dX_t=a(X_t,t)\,dt+b(X_t,t)\,dW_t$ &
Forma generale di una SDE con drift $a$ e coefficiente di
diffusione $b$.\\
\hline
\end{longtable}
...
\]
\[
\mathbb{E}[W_t]=0, \qquad \Var(W_t)=t, \qquad
\Cov(W_s,W_t)=\min(s,t).
\]
Il moto browniano geometrico \`{e} definito da
\[
...
\]
Per il moto browniano geometrico, il rendimento logaritmico cumulato
dall'origine al tempo $t$ \`{e}
\[
\log\!\left(\frac{S_t}{S_0}\right)=\mu t+\sigma W_t
\sim\mathcal{N}(\mu t,\,\sigma^2 t).
\]
I simboli $R_{t+1}$ e $r_t$ indicano oggetti distinti e non devono essere
usati in modo intercambiabile nella stessa lezione.
