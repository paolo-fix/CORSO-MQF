...
q=\frac{1+r-d}{u-d}.
\]
La misura risk-neutral \`{e} indicata con $\Q$. Nel modello binomiale a un
periodo,
\[
\Q(\hbox{up})=q,
\qquad
...
Se $V_t$ indica il valore di un derivato al tempo $t$, allora la formula
ricorsiva di valutazione \`{e}
\[
V_t=
\frac{1}{1+r}
\E^{\Q}\left[V_{t+1}\mid \F_t\right].
\]
Nel caso a un periodo, questa formula diventa
\[
V_0=
\frac{1}{1+r}
\left(
...
Il payoff terminale di un derivato \`{e} indicato con $H$. Per una call
europea con strike $K$,
\[
H=(S_T-K)^+.
\]

\section{Programmazione lineare e programmazione stocastica}

La convenzione primaria del corso per la programmazione lineare \`{e} la
minimizzazione. La forma standard \`{e}
\[
\min_x \; c'x
\qquad
\hbox{soggetto a}
...
Ax=b,\quad x\geq0.
\]
Il valore ottimo di un problema di ottimizzazione \`{e} indicato con
\[
z^*.
\]
Una soluzione ottima \`{e} indicata con
\[
x^*.
\]

Nei problemi di programmazione stocastica a due stadi, la decisione di
primo stadio \`{e} indicata con $x$, mentre la decisione di secondo stadio
nello scenario $s$ \`{e} indicata con $y_s$. L'insieme degli scenari \`{e}
\[
\mathcal S=\{1,\ldots,N\}.
\]
La probabilit\`{a} dello scenario $s$ \`{e} $p_s$.

Per evitare sovrapposizioni con la probabilit\`{a} risk-neutral $q$ del
modello binomiale, i costi di recourse nello scenario $s$ sono indicati
con $d_s$. Una formulazione discreta a due stadi \`{e}
\[
\min_{x,y_s}
\;
...
Per problemi formulati in minimizzazione, si usano le convenzioni
\[
z^{SP}
=
...
z^{EV}
=
\hbox{valore ottenuto dalla soluzione del problema medio}.
\]
Sotto le usuali ipotesi di confronto,
\[
...
\EVPI=z^{SP}-z^{WS}\geq0.
\]
Il valore della soluzione stocastica \`{e}
\[
\VSS=z^{EV}-z^{SP}\geq0.
\]

\section{Corrispondenza tra notazione matematica e nomi Python}
...
$z^{EV}$ & \texttt{z\_ev} & Valore della soluzione del problema medio.\\
...
codice deve richiamare direttamente la formulazione matematica presentata
nel manuale.
