...
\texttt{MQF\_Notazione.tex}. In prima approssimazione, e in coerenza con il documento
\texttt{MQF\_Notazione.tex}, si adottano le seguenti convenzioni generali:
\begin{center}%
...
\multicolumn{1}{l}{$\mathcal S=\{1,\ldots,N\}$} & Insieme finito degli scenari.\\
\multicolumn{1}{l}{$s\in\mathcal S$, $p_s$} & Scenario generico e probabilit\`{a} dello scenario $s$.\\
\hline
\end{tabular}
\end{center}
...
q=\frac{(1+r)-d}{u-d}.
\]
Il valore di un derivato con valori successivi $V_{t+1}^{u}$ e
$V_{t+1}^{d}$ \`{e} calcolato per induzione all'indietro mediante
\[
V_t=
\frac{1}{1+r}
\left(
qV_{t+1}^{u}+(1-q)V_{t+1}^{d}
\right).
\]
\textbf{Esercizi previsti:} albero binomiale dei prezzi;
...
La convenzione primaria del corso per la programmazione lineare \`{e} la
formulazione di minimizzazione. In forma standard:
\[
\min_x \; c'x
\qquad
\hbox{soggetto a}
...
Ax=b,\quad x\geq 0.
\]
Formulazioni equivalenti con vincoli di disuguaglianza o con funzione
obiettivo di massimo saranno introdotte come trasformazioni della forma
standard.
\newline
\textbf{Esercizi previsti:} problema di allocazione di portafoglio con vincoli; rappresentazione grafica in due variabili; identificazione della soluzione ottima.\newline
\textbf{Grafici previsti:} regione ammissibile; rette di livello; soluzione ottima su un vertice.\newline
\textbf{Applicazioni Python collegate:} Lezione 13 e Lezione 16.\newline
\textbf{Stato di avanzamento:} bozza progettuale predisposta; convenzione primaria di minimizzazione adottata.
...
.\newline\textbf{Formule principali:}
Per coerenza con la convenzione primaria di minimizzazione, si considera
inizialmente il problema primale
\[
\min_x \; c'x
\qquad
\hbox{soggetto a}
...
\]
Il corrispondente problema duale \`{e}
\[
\max_y \; b'y
\qquad
\hbox{soggetto a}
\qquad
A'y\leq c,\quad y\geq0.
\]
In forma standard con vincoli di uguaglianza,
\[
\min_x \; c'x
\qquad
\hbox{soggetto a}
...
Ax=b,\quad x\geq0,
\]
le variabili duali associate ai vincoli di uguaglianza sono libere di segno.
...
variabili ausiliarie $z_s$, insieme degli scenari
$\mathcal S=\{1,\ldots,N\}$, probabilit\`{a} $p_s$.\newline
\textbf{Formule principali:}
\[
\min_{x,\eta,z} \;
\eta+\frac{1}{1-\alpha}
\sum_{s\in\mathcal S}p_s z_s
\]
\[
\hbox{soggetto a}
\qquad
...
$x$, decisione di secondo stadio $y_{s}$.\newline\textbf{Notazione introdotta:} insieme degli scenari
$\mathcal S=\{1,\ldots,N\}$, scenario $s\in\mathcal S$,
probabilit\`{a} $p_s$, decisione di primo stadio $x$, decisione di
secondo stadio $y_s$, costo di recourse $d_s$.\newline
\textbf{Formule principali:}
\[
\min_x \; c'x+\E_{\xi}[Q(x,\xi)].
\]
Nel caso di approssimazione discreta a scenari:
\[
\min_{x,y_s} \;
c'x+\sum_{s\in\mathcal S}p_s d_s'y_s
\]
\[
\hbox{soggetto a}
\qquad
...
$\operatorname{VSS}$.\newline\textbf{Formule principali:}
Per problemi formulati in minimizzazione, si assumono le convenzioni
\[
z^{SP}
=
\hbox{valore ottimo del problema stocastico},
\qquad
z^{WS}
=
\sum_{s\in\mathcal S}p_s z_s^{WS},
\]
\[
z^{EV}
=
\hbox{valore ottenuto applicando la soluzione del problema medio
al problema stocastico}.
\]
Sotto le usuali ipotesi di confronto tra informazione perfetta,
soluzione stocastica e soluzione basata sul problema medio, vale
\[
z^{WS}\leq z^{SP}\leq z^{EV}.
\]
Il valore atteso dell'informazione perfetta \`{e}
\[
\EVPI=z^{SP}-z^{WS}\geq0,
\]
...
\]
\newline
\textbf{Esercizi previsti:} calcolo di EVPI in un esempio discreto;
confronto tra soluzione deterministica equivalente e soluzione stocastica;
...
\textbf{Stato di avanzamento:} bozza progettuale predisposta; convenzioni di
segno per EVPI e VSS adottate per problemi di minimizzazione.
