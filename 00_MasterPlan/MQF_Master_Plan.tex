...
\item programmazione lineare, dualit\`a e prezzi ombra;

\item Asset--Liability Management deterministico: cash flow temporali,
bilanci di liquidit\`a e funding;

\item consolidamento computazionale in Python della programmazione lineare
e dell'ALM deterministico;
...
seguenti convenzioni generali. Le integrazioni relative al tempo continuo e
all'Asset--Liability Management deterministico devono essere verificate nel
file di notazione prima della scrittura definitiva dei capitoli
corrispondenti.
...
\multicolumn{1}{l}{$g_{t}$} & Giacenza di liquidit\`a residua alla data $t$
nei modelli ALM.\\
\multicolumn{1}{l}{$u_{t}$, $\bar u_{t}$} & Funding raccolto alla data $t$ e
relativa capacit\`a massima.\\
...
12 & P & Dualit\`a e Asset--Liability Management deterministico &
Cash flow temporali, bilanci di liquidit\`a, funding e valori marginali &
Capitolo teorico, esercizi e grafici \\
13 & C & Applicazione in Python: programmazione lineare e ALM deterministico &
Formulazione per il solver, soluzione, controlli e analisi what-if &
Notebook Jupyter, tracciato IA e slides applicative \\
...
\subsection{Lezione 12 -- Dualit\`a e Asset--Liability Management
deterministico}

\textbf{Tipo:} P.\newline
\textbf{Domanda guida:} come cambia una decisione bancaria quando la
liquidit\`a deve essere disponibile nelle specifiche date in cui sorgono i
fabbisogni?\newline
\textbf{Prerequisiti:} Lezione 11; programmazione lineare, dualit\`a,
prezzi ombra e sensitivit\`a locale.\newline
\textbf{Obiettivi didattici:} distinguere liquidit\`a aggregata e temporale;
rappresentare i cash flow degli attivi; introdurre giacenze e funding;
formulare un ALM deterministico come programma lineare; interpretare i
valori marginali della liquidit\`a alle diverse date; preparare il passaggio
dalla traiettoria deterministica all'incertezza.\newline
\textbf{Caso guida:} estensione multiperiodale del caso SVB introdotto nella
Lezione 11, con cash flow distribuiti nel tempo, fabbisogni, giacenze e
capacit\`a limitata di funding.\newline
\textbf{Notazione introdotta:} $A^{\mathrm{CF}}=(a_{tj})$, $d_t$, $g_t$,
$u_t$, $r_t^{g}$, $r_t^{u}$, $\bar u_t$, $\lambda_0$, $\lambda_t$,
$\lambda_{u,t}$, $\Pi$, $v$; eccezioni locali dichiarate: $X_t$ (stato
patrimoniale della banca), $s_t$ (indicatore di solidit\`a).\newline
\textbf{Grafici previsti:} liquidit\`a aggregata e temporale; cash flow e
fabbisogni; bilancio intertemporale; profili ottimi; valori marginali per
scadenza.\newline
\textbf{Applicazioni Python collegate:} Lezione 13.

\subsection{Lezione 13 -- Applicazione in Python: programmazione lineare
e ALM deterministico}

\textbf{Tipo:} C.\newline
\textbf{Domanda guida:} come si traduce un modello di programmazione lineare
e ALM nella struttura numerica richiesta da un solver e come se ne
controllano e interpretano i risultati?\newline
\textbf{Prerequisiti:} Lezioni 11--12; basi operative di Python.\newline
\textbf{Obiettivi didattici:} costruire vettore delle variabili, funzione
obiettivo, matrici di uguaglianza e disuguaglianza e bounds; risolvere il
problema con un solver LP; controllare bilanci, residui, slack e vincoli
attivi; interpretare i valori marginali; svolgere analisi what-if
controllate.\newline
\textbf{Strumento computazionale:} \texttt{scipy.optimize.linprog} con
metodo \texttt{highs}.\newline
\textbf{Caso applicativo guida:} implementazione di un problema ALM
deterministico coerente con il modello della Lezione 12.\newline
\textbf{Contenuti non sviluppati:} nuova teoria di ottimizzazione o nuova
teoria ALM.\newline
...
Lezioni 11--13.
\newline\textbf{Obiettivi didattici:} introdurre decisioni sotto incertezza, scenari,
modelli a due stadi, decisioni here-and-now e di recourse, funzione di
recourse e forma estesa; distinguere fattibilit\`a del primo stadio e del
recourse; introdurre SP, WS, EV, EVPI e VSS.
...
\newline\textbf{Casi guida e costruzione degli scenari:} Il primo caso, SVB--ALM, lo stato
economico-finanziario esogeno \`e rappresentato da
$Z_t=(Z_t^R,Z_t^C,Z_t^F)\in\{0,1\}^3$ e evolve secondo una catena di Markov
omogenea con matrice $P=(p_{ij})$. Dato lo stato corrente, una riga di $P$
fornisce le probabilit\`a degli otto scenari del secondo stadio. Nel caso
Alcoa gli scenari sono invece ottenuti simulando $10000$ traiettorie e
riducendole a $20$ mediante Fast Forward Selection con funzione di distanza basata sull'impatto
sulla funzione obiettivo (Variant 2 in Conejo, Carri\'{o}n e Morales).

Il secondo caso \`e una costruzione didattica stilizzata
ispirata al problema di approvvigionamento elettrico dello smelter Alcoa
San Cipri\'{a}n: la decisione di primo stadio riguarda la quantit\`a coperta
contrattualmente ex ante, mentre dopo l'osservazione delle condizioni di
prezzo e fabbisogno sono ammesse decisioni correttive di approvvigionamento.
Per la struttura del problema di procurement di un grande consumatore il
riferimento scientifico \`e Conejo, Carri\'{o}n e Morales, Capitolo 9; per la
riduzione degli scenari \`e il Capitolo 3 dello stesso testo. Nel caso Alcoa
si generano $10000$ traiettorie e se ne selezionano $20$ mediante Fast
Forward Selection con distanza dipendente dal problema decisionale
(Variant 2 in Conejo, Carri\'{o}n e Morales). I $20$ scenari sono utilizzati
nel modello a due stadi e non costituiscono un albero multistadio.\\newline
\newline\textbf{Specificazione SVB--ALM:} $X_t$ indica lo stato patrimoniale e
finanziario della banca; i fabbisogni soddisfano $d_t=d(Z_t)$ e la solidit\`a
\`e rappresentata da
\[
\kappa_t=\kappa_0+\beta'X_t-h_Z(Z_t),
\]
con
\[
h_Z(Z_t)=
\gamma_R Z_t^R+\gamma_C Z_t^C+\gamma_F Z_t^F
+\gamma_I\left(
Z_t^RZ_t^C+Z_t^RZ_t^F+Z_t^CZ_t^F
\right).
\]
\newline\textbf{Formule
principali:}
...
principali:}
\[
x_t^{s}=x_t^{s^{\prime}} \qquad\hbox{se gli scenari }s\hbox{ e }s^{\prime
}\hbox{ hanno la stessa storia fino a }t.
\]
Per problemi formulati in massimizzazione (convenzione adottata nel caso
guida SVB--ALM),
\[
z^{WS}\geq z^{SP}\geq z^{EV}, \qquad\operatorname{EVPI}=z^{WS}-z^{SP},
\qquad\operatorname{VSS}=z^{SP}-z^{EV}.
\]
...
12 & Formulazione ALM da descrizione finanziaria &
Fattibilit\`a temporale e funding &
Valori duali e forma matriciale per il solver\\
13 & Costruzione e soluzione del modello LP &
Controlli, slack e valori marginali &
Profili ALM e analisi what-if\\
...
4 & 13 & Programmazione lineare e ALM deterministico &
Costruzione delle matrici del solver, soluzione LP, controlli, valori
marginali e analisi what-if. \\
