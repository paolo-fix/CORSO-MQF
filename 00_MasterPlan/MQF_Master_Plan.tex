...
numero di repliche simulate; interpretare la stabilit\`{a} numerica.\newline%
...
\pi_t=\pi_0P^t,
\qquad
\widehat{\Prob}(\tau\leq T)=\frac{1}{M}\sum_{k=1}^{M}D_k,
\qquad
D_k=\1_{\{\tau^{(k)}\leq T\}}.
\]
dove $\tau^{(k)}$ \`{e} il tempo di default simulato nella replica $k$ e
$D_k$ \`{e} l'indicatrice dell'evento di default entro l'orizzonte $T$.
