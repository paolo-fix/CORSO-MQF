% !TeX root = ../MQF_Manuale_Master.tex
%2multibyte Version: 5.50.0.2960 CodePage: 65001
%TCIDATA{OutputFilter=latex2.dll}
%TCIDATA{Version=5.50.0.2960}
%TCIDATA{LaTeXparent=1,1,MQF_Manuale_Master.tex}


\chapter{Elementi di teoria della probabilit\`{a}}

\label{cap:probabilita}

\section{Obiettivi della lezione}

La finanza quantitativa studia grandezze economiche e finanziarie che non sono
note con certezza nel momento in cui una decisione viene assunta. Il
rendimento futuro di un titolo, il valore di un portafoglio alla fine di un
periodo, l'eventuale default di un debitore, la variazione di un tasso di
interesse o il superamento di una soglia di perdita sono esempi di fenomeni il
cui esito dipende da condizioni future non pienamente osservabili al tempo
della decisione.

Il primo obiettivo di questo capitolo \`{e} introdurre il linguaggio
matematico di base con cui tale incertezza viene rappresentata. Questo
linguaggio \`{e} quello della teoria della probabilit\`{a}. In particolare, il
capitolo introduce gli elementi costitutivi di uno spazio di probabilit\`{a}:
\[
(\Omega,\F,\Prob).
\]
L'insieme $\Omega$ rappresenta gli esiti elementari possibili, $\F$
rappresenta la famiglia degli eventi ai quali il modello assegna significato
probabilistico, mentre $\Prob$ assegna a ciascun evento una probabilit\`{a}.

Il secondo obiettivo \`{e} chiarire come si calcolano le probabilit\`{a} di
eventi composti e come si interpreta il condizionamento. In finanza, il
condizionamento \`{e} essenziale: un segnale di mercato, un deterioramento del
rating, una variazione dello spread o un nuovo dato macroeconomico modificano
l'informazione disponibile e quindi la valutazione probabilistica degli eventi futuri.

Al termine del capitolo lo studente dovr\`{a} essere in grado di:

\begin{itemize}
\item distinguere tra esiti elementari, eventi e famiglie di eventi;

\item interpretare uno spazio di probabilit\`{a} in un contesto finanziario;

\item utilizzare le operazioni fondamentali sugli eventi;

\item applicare le regole elementari della probabilit\`{a};

\item calcolare probabilit\`{a} condizionate;

\item distinguere tra eventi disgiunti ed eventi indipendenti;

\item applicare la formula delle probabilit\`{a} totali e la formula di Bayes;

\item interpretare l'aggiornamento bayesiano come revisione della
probabilit\`{a} in presenza di nuova informazione.
\end{itemize}

Il capitolo prepara il passaggio al Capitolo 2. In questa fase l'attenzione
\`{e} posta sugli eventi. Nel capitolo successivo gli scenari saranno
trasformati in grandezze numeriche mediante il concetto di variabile casuale.

\section{Motivazione finanziaria: incertezza, scenari ed eventi}

Consideriamo un investitore che osserva oggi il prezzo di un titolo e deve
valutare il possibile valore del proprio investimento alla fine del periodo
successivo. Al tempo iniziale, indicato con $t=0$, il valore futuro non \`{e}
noto. Esistono diversi scenari compatibili con l'informazione disponibile: il
prezzo potrebbe aumentare, rimanere sostanzialmente stabile oppure diminuire.

In termini elementari, possiamo rappresentare questi scenari mediante uno
spazio campionario finito:
\[
\Omega=\{\omega_{1},\omega_{2},\omega_{3}\},
\]
dove $\omega_{1}$ rappresenta uno scenario di ribasso, $\omega_{2}$ uno
scenario di stabilit\`{a} e $\omega_{3}$ uno scenario di rialzo. Lo spazio
$\Omega$ non \`{e} ancora un modello probabilistico completo: esso elenca
soltanto gli esiti possibili.

Se definiamo
\[
A=\{\omega_{1}\},
\]
l'evento $A$ pu\`{o} essere interpretato come evento di perdita. Se definiamo
\[
B=\{\omega_{2},\omega_{3}\},
\]
l'evento $B$ rappresenta la circostanza in cui il rendimento non \`{e}
negativo. L'evento certo \`{e} $\Omega$, mentre l'evento impossibile \`{e}
$\emptyset$.

Un secondo esempio riguarda il rischio di credito. Un debitore pu\`{o}
rimborsare regolarmente oppure andare in default. In una rappresentazione
elementare si pu\`{o} porre
\[
\Omega=\{\omega_{N},\omega_{D}\},
\]
dove $\omega_{N}$ indica lo stato di non default e $\omega_{D}$ indica lo
stato di default. L'evento di default \`{e}
\[
D=\{\omega_{D}\}.
\]
La probabilit\`{a} $\Prob(D)$ rappresenta la probabilit\`{a} di default
sull'orizzonte temporale considerato. Tale probabilit\`{a}, tuttavia, pu\`{o}
essere rivista se si osserva un nuovo segnale informativo, ad esempio un
peggioramento del rating o un aumento dello spread.

Questi esempi mostrano che un modello probabilistico richiede tre elementi:
\[
\text{scenari possibili}\quad\longrightarrow\quad\text{eventi rilevanti}%
\quad\longrightarrow\quad\text{probabilit\`{a} degli eventi}.
\]
La tripla $(\Omega,\F,\Prob)$ formalizza precisamente questi tre elementi.

\section{Spazio campionario ed eventi}

\begin{definition}
Lo spazio campionario $\Omega$ \`{e} l'insieme degli esiti elementari
possibili di un esperimento aleatorio. Un elemento $\omega\in\Omega$ \`{e}
detto evento o esito elementare.
\end{definition}

Nel contesto finanziario, gli esiti elementari possono rappresentare scenari
di mercato, stati di credito, evoluzioni di un tasso di interesse oppure
possibili realizzazioni di un insieme di fattori di rischio.

\begin{definition}
Un evento \`{e} un sottoinsieme dello spazio campionario. Se $A\subseteq
\Omega$, dire che l'evento $A$ si verifica significa che l'esito elementare
realizzato appartiene ad $A$.
\end{definition}

Nel caso di uno spazio finito, ad esempio
\[
\Omega=\{\omega_{1},\omega_{2},\omega_{3},\omega_{4}\},
\]
un evento pu\`{o} essere
\[
A=\{\omega_{1},\omega_{2}\}.
\]
Se $\omega_{1}$ o $\omega_{2}$ si realizza, allora si dice che $A$ si \`{e}
verificato. Se invece si realizza $\omega_{3}$ o $\omega_{4}$, l'evento $A$
non si \`{e} verificato. 
Un caso particolare, ma perfettamente ammissibile, \`{e} quello in cui un evento $A$
contiene un solo esito elementare. Se $\omega\in\Omega$, l'insieme
\[
A=\{\omega\}
\]
\`{e} detto \textbf{evento elementare}, o \textbf{singleton}. Esso rappresenta il caso in
cui si verifica esattamente l'esito $\omega$. Gli eventi singleton sono gli eventi pi\`{u} fini che si possano considerare:
non possono essere ulteriormente scomposti in eventi non vuoti pi\`{u} piccoli.
Una partizione formata da soli singleton distingue ogni esito elementare di
$\Omega$.

Altri due casi particolari sono l'\textbf{evento certo} e l'\textbf{evento impossibile}. 
L'evento certo coincide con l'intero spazio degli esiti:
\[
\Omega.
\]
Esso si verifica qualunque sia l'esito realizzato. L'evento impossibile coincide
invece con l'insieme vuoto:
\[
\emptyset.
\]
Esso non si verifica per alcun esito elementare.

In questo capitolo useremo soprattutto spazi campionari finiti o discreti. Gli
spazi continui saranno richiamati solo quando necessario e saranno ripresi nel
Capitolo 2, quando introdurremo variabili casuali e distribuzioni.

\section{Spazi campionari generati da esperimenti ripetuti}

Molti problemi finanziari non riguardano un singolo esperimento aleatorio, ma
una sequenza di esperimenti osservati nel tempo. Si pensi, ad esempio, ai
movimenti giornalieri di un prezzo, all'evoluzione periodica del rating di una
controparte, alla successione di rialzi e ribassi del valore di un
portafoglio, oppure alla simulazione di una traiettoria di mercato su pi\`{u}
date future.

Supponiamo che l'incertezza sia osservata in $n$ date successive e che, alla
data $i=1,\ldots,n$, gli esiti possibili siano descritti da uno spazio
campionario $\Omega_{i}$. Lo spazio campionario complessivo \`{e} allora il
prodotto cartesiano
\[
\Omega=\Omega_{1}\times\Omega_{2}\times\cdots\times\Omega_{n}.
\]
Un esito elementare $\omega\in\Omega$ \`{e} una $n$-pla
\[
\omega=(\omega_{1},\omega_{2},\ldots,\omega_{n}),
\]
dove $\omega_{i}\in\Omega_{i}$ rappresenta l'esito elementare realizzato nella
$i$-esima osservazione.

Questa notazione \`{e} particolarmente importante per il seguito del manuale.
Essa permette di interpretare un esito elementare non come un singolo stato
isolato ad una sola data $i$, ma come una \textit{sequenza ordinata} di
realizzazioni. In questo senso, un elemento $\omega\in\Omega$ pu\`{o} essere
letto come una \textit{traiettoria} elementare.

Nel caso in cui lo stesso esperimento sia ripetuto $n$ volte e ciascuno spazio
degli esiti sia lo stesso a ogni data, cio\`{e}
\[
\Omega_{1}=\Omega_{2}=\cdots=\Omega_{n}=\Omega^{\ast},
\]
si scrive pi\`{u} semplicemente
\[
\Omega=\underset{\text{n volte}}{\underbrace{\Omega^{\ast}\times\Omega^{\ast
}\times...\times\Omega^{\ast}}}=(\Omega^{\ast})^{n}.
\]
Se $\Omega^{\ast}$ contiene $m$ esiti elementari, allora lo spazio prodotto
$(\Omega^{\ast})^{n}$ contiene
\[
m^{n}%
\]
esiti elementari.

\begin{example}
Consideriamo tre movimenti successivi del prezzo di un titolo. A ogni data il
prezzo pu\`{o} registrare un rialzo, indicato con $u$, oppure un ribasso,
indicato con $d$. Lo spazio degli esiti di una singola osservazione \`{e}
\[
\Omega^{\ast}=\{u,d\}.
\]
Per tre osservazioni successive, lo spazio campionario \`{e}
\[
\Omega=(\Omega^{\ast})^{3}=\{u,d\}^{3}.
\]
Gli esiti elementari sono le otto sequenze
\[
\Omega=\{uuu,uud,udu,udd,duu,dud,ddu,ddd\}.
\]
Ad esempio,
\[
\omega=udd
\]
rappresenta la traiettoria in cui il primo movimento \`{e} un rialzo, mentre
il secondo e il terzo sono ribassi.
\end{example}

In questa rappresentazione, gli eventi sono sottoinsiemi di sequenze. Ad
esempio, nell'esempio precedente, l'evento
\[
A=\{uuu,uud,udu,udd\}
\]
rappresenta la circostanza in cui il primo movimento \`{e} un rialzo.
L'evento
\[
B=\{udd,dud,ddu\}
\]
rappresenta invece la circostanza in cui si osserva esattamente un rialzo nei
tre periodi. L'evento
\[
C=\{uuu,uud,udu,duu\}
\]
rappresenta la circostanza in cui si osservano almeno due rialzi.

Questi esempi mostrano che, quando gli esiti elementari sono sequenze, gli
eventi possono descrivere propriet\`{a} della traiettoria: il primo movimento,
il numero totale di rialzi, il verificarsi di due ribassi consecutivi, oppure
il superamento di una soglia lungo il percorso.

\begin{example}
Consideriamo ora quattro date trimestrali di osservazione del merito
creditizio di una controparte. A ogni trimestre si osserva se il merito
creditizio rimane stabile, indicato con $s$, oppure si deteriora, indicato con
$d$. Lo spazio degli esiti di una singola osservazione \`{e}
\[
\Omega^{\ast}=\{s,d\}.
\]
Su quattro trimestri,
\[
\Omega=(\Omega^{\ast})^{4}=\{s,d\}^{4},
\]
e quindi lo spazio campionario contiene
\[
2^{4}=16
\]
traiettorie elementari.

L'esito
\[
\omega=sdds
\]
rappresenta una traiettoria nella quale il merito creditizio resta stabile nel
primo trimestre, si deteriora nel secondo e nel terzo, e torna stabile nel quarto.

L'evento
\[
A=\{\omega\in\Omega:\text{ in }\omega\text{ compaiono almeno due simboli }d\}
\]
rappresenta la circostanza in cui si osservano almeno due deterioramenti nel
periodo considerato. Questo evento non riguarda un singolo trimestre, ma una
propriet\`{a} dell'intera traiettoria.
\end{example}

La stessa logica si estende al caso di un numero infinito di osservazioni. Se
l'esperimento viene ripetuto indefinitamente e lo spazio elementare \`{e}
$\Omega^{\ast}$, si pu\`{o} considerare lo spazio
\[
\Omega=(\Omega^{\ast})^{\mathbb{N}}.
\]
In questo caso un esito elementare \`{e} una successione infinita
\[
\omega=(\omega_{1},\omega_{2},\ldots).
\]
Nel presente manuale lavoreremo prevalentemente con orizzonti finiti
$t=0,1,\ldots,T$, ma la notazione per successioni infinite \`{e} utile per
comprendere il legame con modelli stocastici pi\`{u} generali.

Questa costruzione sar\`{a} ripresa pi\`{u} volte. Nei processi stocastici,
una traiettoria descrive l'evoluzione temporale di una variabile aleatoria.
Negli alberi binomiali, ogni percorso dalla \textit{radice}, alla \textit{foglia},
rappresenta una sequenza di rialzi e ribassi. Nella simulazione, ogni replica
genera una traiettoria casuale. Nella programmazione stocastica, uno scenario
pu\`{o} essere interpretato come una realizzazione coerente dell'incertezza
lungo l'orizzonte temporale considerato.

\section{Operazioni sugli eventi}

Siano $A,B\subseteq\Omega$ due eventi.

L'\textit{unione}
\[
A\cup B
\]
\`{e} l'evento che si verifica quando si verifica almeno uno tra $A$ e $B$.
L'\textit{intersezione}
\[
A\cap B
\]
\`{e} l'evento che si verifica quando si verificano simultaneamente $A$ e $B$.
Il complemento
\[
A^{c}=\Omega\setminus A
\]
\`{e} l'evento che si verifica quando $A$ non si verifica.

Due eventi $A$ e $B$ si dicono \textbf{disgiunti o incompatibili} se
\[
A\cap B=\emptyset.
\]
In questo caso non possono verificarsi simultaneamente. 

Se invece
\[
A\cap B\neq\emptyset,
\]
gli eventi si dicono \textbf{compatibili}: esistono esiti elementari che appartengono
sia ad $A$ sia a $B$, e quindi i due eventi possono verificarsi congiuntamente.

\begin{example}
Supponiamo che $\Omega=\{\omega_{1},\omega_{2},\omega_{3}\}$ rappresenti tre
scenari di mercato: ribasso, stabilit\`{a} e rialzo. Se
\[
A=\{\omega_{1}\},\qquad B=\{\omega_{1},\omega_{2}\},
\]
allora $A$ rappresenta l'evento di perdita, mentre $B$ rappresenta l'evento in
cui non vi \`{e} rialzo. Si ha
\[
A\cap B=\{\omega_{1}\},\qquad A\cup B=\{\omega_{1},\omega_{2}\},\qquad
A^{c}=\{\omega_{2},\omega_{3}\}.
\]

\end{example}

\section{Famiglie di eventi}
In molte situazioni non interessa considerare un singolo evento, ma un insieme di eventi accomunati da una certa propriet\`{a}. 
Una famiglia di eventi arbitrari, che possono essere disgiunti o congiunti, \`{e} un insieme del tipo
\[
\mathcal{A}=\{A_i:i\in I\},
\]
dove ogni \(A_i\) \`{e} un evento.
Occorre distinguere accuratamente la famiglia di eventi
\[
\mathcal{A}=\{A_i:i\in I\}
\]
dall'evento ottenuto unendo gli eventi che la compongono:
\[
\mathcal{A}\neq \bigcup_{i\in I}A_i.
\]
La famiglia \(\mathcal{A}\) \`{e} un insieme i cui elementi sono eventi, ma non \`{e} un evento; l'unione \(\bigcup_{i\in I}A_i\), invece, \`{e} un sottoinsieme di \(\Omega\), e quindi \`{e} un evento. 

\begin{example}
Sia
\[
\Omega=\{1,2,3,4,5,6\}
\]
lo spazio dei risultati del lancio di un dado. Consideriamo gli eventi
\[
A=\{2,4,6\},\qquad B=\{1,3,5\}.
\]
La famiglia
\[
\mathcal{A}=\{A,B\}
\]
ha come elementi due eventi: l'evento ``esce un numero pari'' e l'evento ``esce un numero dispari''. 
Essa non coincide con l'evento
\[
A\cup B=\{1,2,3,4,5,6\}=\Omega.
\]
Infatti \(\mathcal{A}\) \`{e} un insieme di sottoinsiemi di \(\Omega\), mentre \(A\cup B\) \`{e} un sottoinsieme di \(\Omega\). 
La probabilit\`{a} \(P(A\cup B)\) \`{e} ben definita; al contrario, \(P(\mathcal{A})\) non ha significato nello spazio di probabilit\`{a} originario, perch\'{e} \(\mathcal{A}\) non \`{e} un evento di \(\F\), ma una collezione di eventi.
\end{example}

In altre parole:
\[
\mathcal{A}=\{A,B\}\neq A\cup B.
\]
e
\[
\mathcal{A}=\{A_i:i\in I\},
\qquad
A_i\subseteq\Omega,
\qquad
\bigcup_{i\in I}A_i\subseteq\Omega.
\]
Le famiglie di eventi permettono di descrivere in modo compatto classi di eventi, operazioni collettive e strutture logiche. 
Per esempio, data una famiglia \(\{A_i:i\in I\}\), l'evento
\[
\bigcup_{i\in I}A_i
\]
rappresenta il verificarsi di almeno uno degli eventi \(A_i\), mentre
\[
\bigcap_{i\in I}A_i
\]
rappresenta il verificarsi simultaneo di tutti gli eventi della famiglia. 
Analogamente,
\[
\left(\bigcup_{i\in I}A_i\right)^c
\]
rappresenta l'evento in cui nessuno degli eventi \(A_i\) si verifica.

\subsection{Partizione di $\Omega$}
Una importante famiglia di eventi \`{e} la \textbf{partizione di $\Omega$}.

\begin{definition}
Una famiglia di eventi $B_{1},\ldots,B_{n}$ \`{e} una partizione di $\Omega$
se gli eventi sono due a due disgiunti e la loro unione \`{e} l'intero spazio
campionario:
\[
B_{i}\cap B_{j}=\emptyset\quad(i\neq j),\qquad\bigcup_{i=1}^{n}B_{i}=\Omega.
\]

\end{definition}

Le partizioni sono importanti perch\'{e} permettono di decomporre un problema
probabilistico rispetto a scenari alternativi e mutuamente esclusivi.

\subsection{Informazione e $\sigma$-algebre}

Un'altra famiglia di eventi estremamente importante \`{e} la cosiddetta $\sigma$-algebra.
\begin{definition}
Una $\sigma$-algebra $\F$ su $\Omega$ \`{e} una famiglia di sottoinsiemi
di $\Omega$ tale che:

\begin{enumerate}
\item $\Omega\in\F$;

\item se $A\in\F$, allora $A^{c}\in\F$;

\item se $A_{1},A_{2},\ldots$ appartengono a $\F$, allora
$\bigcup_{n=1}^{\infty} A_{n}\in\F$.
\end{enumerate}
\end{definition}

Gli elementi di $\F$ sono eventi. L'interpretazione finanziaria \`{e} la seguente: $\F$ descrive quali
eventi sono distinguibili nel modello. Una sigma-algebra pi\`{u} ricca
consente di distinguere pi\`{u} eventi; una sigma-algebra pi\`{u} povera
rappresenta un livello informativo pi\`{u} aggregato. Pi\`{u} in generale, una $\sigma$-algebra pu\`{o} essere interpretata come una rappresentazione matematica dell'informazione disponibile. Condizionare rispetto a un evento, o rispetto a una famiglia strutturata di eventi, significa allora ricalcolare le probabilit\`{a} alla luce dell'informazione espressa da quegli eventi. 

La $\sigma$-algebra pi\`{u} povera che si possa definire su $\Omega$ \`{e}
\[
\F_{\min}=\{\emptyset,\Omega\}.
\]
Essa contiene soltanto l'evento impossibile e l'evento certo. 
Non distingue quindi alcun sottoinsieme proprio di $\Omega$ come evento osservabile. 
Dal punto di vista informativo, questa $\sigma$-algebra rappresenta il livello
minimo di informazione: consente solo di affermare che l'esito appartiene a
$\Omega$, ma non permette di distinguere tra regioni diverse dello spazio degli esiti.

\subsection{Partizioni e $\sigma$-algebre generate}

Nella pratica si considerano di norma $\sigma$-algebre pi\`{u} informative
della $\sigma$-algebra banale $\{\emptyset,\Omega\}$. 
Consideriamo uno spazio degli
esiti $\Omega$ che sia suddiviso mediante una partizione finita
\[
\mathcal{A}=\{A_1,\ldots,A_m\},
\]
i cui eventi rappresentano gli eventi elementari rilevanti per il
problema. A partire dalla partizione $\mathcal{A}$ si pu\`{o} costruire in modo naturale
una e una sola $\sigma$-algebra $\F$ generata dalla partizione. Essa
\`{e} l'unica $\sigma$-algebra che contiene tutti gli elementi della
partizione.

La costruzione della $\sigma$-algebra pu\`{o} essere descritta in modo iterativo:

\begin{enumerate}
\item Si parte dalla collezione iniziale formata dall'evento certo e dagli elementi  della partizione:
\[
\{\Omega,A_1,\ldots,A_m\}.
\]

\item Si arricchisce la collezione aggiungendo i
complementari degli insiemi gi\`{a} presenti, quindi:
\[
\emptyset =\Omega^c, \qquad
A_i^c=\Omega\setminus A_i.
\]

\item Si aggiungono poi tutte le unioni ottenibili dagli insiemi disponibili.
In particolare, per ogni $A_i$ e $A_j$ gi\`{a} appartenenti o aggiunti alla collezione corrente, si aggiunge anche
\[
A_z=A_i\cup A_j.
\]
Pi\`{u} in generale, si aggiungono tutte le unioni finite degli insiemi gi\`{a}
presenti.

\item Dopo avere aggiunto complementari e unioni, si verifica se sono comparsi
nuovi insiemi. Se la collezione si \`{e} ampliata, si ripete la procedura:
si prendono nuovamente i complementari e le unioni degli insiemi ora disponibili. Viceversa, se non si riesce pi\`{u} ad arricchire la collezione, la procedura si ferma e la collezione cos\`{i} realizzata \`{e} la $\sigma$-algebra ricercata.
\end{enumerate}

La collezione finale cos\`{\i} ottenuta contiene $\emptyset$ e $\Omega$, \`{e}
chiusa rispetto al complementare ed \`{e} chiusa rispetto all'unione.  Indichiamo questa $\sigma$-algebra $\F=\sigma(\mathcal{A})$.

\paragraph*{Raffinamento di una partizione.}
Ai fini dei capitoli successivi, \`{e} importante osservare che una
$\sigma$-algebra pi\`{u} povera pu\`{o} essere inclusa in una
$\sigma$-algebra pi\`{u} ricca:
\[
\mathcal{G}\subseteq\F.
\]
Questa inclusione formalizza il passaggio da un livello informativo pi\`{u}
grossolano a uno pi\`{u} dettagliato. Quando le $\sigma$-algebre sono generate
da partizioni finite, tale passaggio corrisponde al raffinamento delle
partizioni di $\Omega$.

Siano $\mathcal{S}$ e $\mathcal{R}$ due partizioni di $\Omega$. 
Si dice che $\mathcal{R}$ \`{e} un raffinamento di $\mathcal{S}$, oppure che
$\mathcal{R}$ \`{e} pi\`{u} fine di $\mathcal{S}$, se ogni blocco di
$\mathcal{R}$ \`{e} contenuto in un blocco di $\mathcal{S}$. 
Formalmente, per ogni $R\in\mathcal{R}$ esiste un $S\in\mathcal{S}$ tale che
\[
R\subseteq S.
\]

In termini informativi, $\mathcal{R}$ distingue almeno tutti i casi distinti da
$\mathcal{S}$, e pu\`{o} distinguerne di ulteriori. Equivalentemente, due esiti
che appartengono a blocchi distinti di $\mathcal{S}$ appartengono anche a
blocchi distinti di $\mathcal{R}$. Se inoltre esistono due esiti che
$\mathcal{R}$ distingue ma che $\mathcal{S}$ non distingue, allora
$\mathcal{R}$ \`{e} un raffinamento proprio di $\mathcal{S}$.

Se $\mathcal{R}$ \`{e} un raffinamento di $\mathcal{S}$, risulta che 
\[
\sigma(\mathcal{S})\subseteq \sigma(\mathcal{R}).
\]

\begin{example}
Sia
\[
\Omega=\{1,2,3,4\}.
\]
Consideriamo la partizione
\[
\mathcal{S}=\big\{\{1,2\},\{3,4\}\big\}.
\]
La $\sigma$-algebra generata da $\mathcal{S}$ \`{e}
\[
\sigma(\mathcal{S})
=
\big\{
\emptyset,
\{1,2\},
\{3,4\},
\Omega
\big\}.
\]
Essa distingue solo due casi: l'esito appartiene a $\{1,2\}$ oppure appartiene
a $\{3,4\}$.

Consideriamo ora la partizione pi\`{u} fine
\[
\mathcal{R}=\big\{\{1\},\{2\},\{3,4\}\big\}.
\]
La partizione $\mathcal{R}$ \`{e} un raffinamento di $\mathcal{S}$: infatti
i blocchi $\{1\}$ e $\{2\}$ sono contenuti nel blocco $\{1,2\}$ di
$\mathcal{S}$, mentre il blocco $\{3,4\}$ coincide con un blocco di
$\mathcal{S}$.

La $\sigma$-algebra generata da $\mathcal{R}$ \`{e}
\[
\sigma(\mathcal{R})
=
\big\{
\emptyset,
\{1\},
\{2\},
\{3,4\},
\{1,2\},
\{1,3,4\},
\{2,3,4\},
\Omega
\big\}.
\]
Si verifica immediatamente che
\[
\sigma(\mathcal{S})\subseteq\sigma(\mathcal{R}).
\]
Infatti gli eventi $\emptyset$, $\{1,2\}$, $\{3,4\}$ e $\Omega$ appartengono
anche a $\sigma(\mathcal{R})$.

La ragione \`{e} che $\mathcal{R}$ contiene una descrizione pi\`{u} dettagliata
di $\Omega$: essa distingue l'esito $1$ dall'esito $2$, mentre
$\mathcal{S}$ li raggruppa nello stesso blocco. Una partizione pi\`{u} fine
genera quindi una $\sigma$-algebra pi\`{u} ricca.
\end{example}



\paragraph*{Insieme delle parti.}
Dato lo spazio degli esiti $\Omega$, si indica con
\[
\mathcal{P}(\Omega)
\]
l'insieme di tutti i sottoinsiemi di $\Omega$:
\[
\mathcal{P}(\Omega)=\{A:A\subseteq\Omega\}.
\]
$\mathcal{P}(\Omega)$ \`{e} costruibile applicando le codizioni di complementazione e di unione previste per una qualunque $\sigma$-algebra partendo da tutti gli eventi elementari di $\Omega$. In questo senso, esso rappresenta la famiglia pi\`{u} ampia di eventi che si possa considerare su $\Omega$.
Ogni altra $\sigma$-algebra di eventi su $\Omega$ sar\`{a} una sottocollezione di $\mathcal{P}(\Omega)$.

Le $\sigma$-algebre su $\Omega$ sono ordinate per inclusione solo parzialmente:
non tutte sono confrontabili tra loro. Tuttavia, quando pi\`{u} $\sigma$-algebre sono confrontabili per inclusione, esse formano
una catena crescente
\[
\F_0\subseteq\F_1\subseteq\cdots\subseteq\F_n \subseteq \mathcal{P}(\Omega),
\]
interpretabile come una successione di livelli informativi progressivamente
pi\`{u} ricchi. 

\begin{example}
Consideriamo tre movimenti successivi di un titolo, ciascuno dei quali pu\`{o}
essere un rialzo $u$ oppure un ribasso $d$. Lo spazio campionario \`{e}
\[
\Omega=\{uuu,uud,udu,udd,duu,dud,ddu,ddd\}.
\]
Prima di osservare i movimenti, l'informazione \`{e} minima. Dopo il primo
movimento si pu\`{o} distinguere tra gli scenari che iniziano con $u$ e quelli
che iniziano con $d$. Dopo due movimenti, la distinzione diventa pi\`{u} fine.
Dopo tre movimenti l'informazione \`{e} completa. Questa successione di
livelli informativi anticipa il ruolo delle filtrazioni $\{\F%
_{t}\}_{t=0}^{T}$, che saranno introdotte nei capitoli successivi.
\end{example}

\section{Misura di probabilit\`{a}}

\begin{definition}
Una misura di probabilit\`{a} su $(\Omega,\F)$ \`{e} una funzione
\[
\Prob:\F\longrightarrow\lbrack0,1]
\]
tale che:

\begin{enumerate}
\item $\Prob(A)\geq0$ per ogni $A\in\F$;

\item $\Prob(\Omega)=1$;

\item se $A_{1},A_{2},\ldots$ sono eventi due a due disgiunti, allora
\[
\Prob\left(  \bigcup_{n=1}^{\infty} A_{n}\right)  = \sum_{n=1}^{\infty
}\Prob(A_{n}).
\]

\end{enumerate}
\end{definition}

Da questi assiomi discendono alcune regole di calcolo elementari. Per ogni
$A,B\in\F$,
\[
\Prob(A^{c})=1-\Prob(A),
\]
e vale la \textbf{regola della somma}:
\[
\Prob(A\cup B)=\Prob(A)+\Prob(B)-\Prob(A\cap B).
\]
Il termine $\Prob(A\cap B)$ viene sottratto perch\'{e} gli esiti appartenenti
ad $A\cap B$ sono conteggiati due volte nella somma
$\Prob(A)+\Prob(B)$.
Se $A$ e $B$ sono disgiunti, allora $A\cap B=\emptyset$ e la formula si riduce a
\[
\Prob(A\cup B)=\Prob(A)+\Prob(B).
\]


\begin{example}
Supponiamo che uno spazio campionario finito sia dato da
\[
\Omega=\{\omega_{1},\omega_{2},\omega_{3}\},
\]
con
\[
\Prob(\{\omega_{1}\})=0.25,\qquad\Prob(\{\omega_{2}\})=0.35,\qquad
\Prob(\{\omega_{3}\})=0.40.
\]
Se $A=\{\omega_{1}\}$ e $B=\{\omega_{2},\omega_{3}\}$, allora
\[
\Prob(A)=0.25,\qquad\Prob(B)=0.35+0.40=0.75.
\]

\end{example}

\section{Spazio di probabilit\`{a}}

\begin{definition}
Uno spazio di probabilit\`{a} \`{e} una tripla
\[
(\Omega,\F,\Prob),
\]
dove $\Omega$ \`{e} lo spazio campionario, $\F$ \`{e} una
sigma-algebra di eventi e $\Prob$ \`{e} una misura di probabilit\`{a} su
$\F$.
\end{definition}

Questa tripla separa tre piani concettuali distinti: gli eventi elementari possibili,
gli eventi \textbf{osservabili} o \textbf{modellizzati}, e la probabilit\`{a} assegnata a tali
eventi. La distinzione \`{e} essenziale in finanza, perch\'{e} una stessa
struttura di scenari pu\`{o} essere valutata con misure di probabilit\`{a}
diverse, ad esempio una misura storica oppure, nei modelli di pricing, una
misura risk-neutral $\Q$.

\section{Probabilit\`{a} condizionata}

\begin{definition}
Siano $A,B\in\F$ e sia $\Prob(B)>0$. La probabilit\`{a}
condizionata di $A$ dato $B$ \`{e} definita da
\[
\Prob(A\mid B)=\frac{\Prob(A\cap B)}{\Prob(B)}.
\]

\end{definition}

La probabilit\`{a} condizionata rappresenta la probabilit\`{a} dell'evento $A$
quando si assume noto che l'evento $B$ si sia verificato. In questa
prospettiva, $B$ restringe lo spazio informativo rilevante.

Dalla definizione segue la \textbf{regola del prodotto}:
\[
\Prob(A\cap B)=\Prob(A\mid B)\Prob(B).
\]
Simmetricamente, se $\Prob(A)>0$,
\[
\Prob(A\cap B)=\Prob(B\mid A)\Prob(A).
\]


\begin{example}
Questo esempio ha funzione qualitativa. Sia $D$ l'evento di default di un
debitore e sia $S$ l'evento che rappresenta l'osservazione di un segnale
negativo di mercato. La probabilit\`{a}
\[
\Prob(D\mid S)
\]
misura la probabilit\`{a} di default dopo avere osservato il segnale negativo.

In generale, $\Prob(D\mid S)$ pu\`{o} essere molto diversa dalla
probabilit\`{a} non condizionata $\Prob(D)$. Il segnale $S$ modifica
infatti l'informazione disponibile e restringe l'attenzione agli scenari nei
quali tale segnale si \`{e} verificato.
\end{example}

\begin{example}
Consideriamo due esposizioni creditizie appartenenti allo stesso portafoglio.
Per ciascuna esposizione si osserva, su un dato orizzonte temporale, se vi
\`{e} stato un deterioramento del merito creditizio oppure no. Indichiamo con
$D$ il deterioramento e con $N$ l'assenza di deterioramento. Supponiamo, in
forma puramente stilizzata, che i quattro esiti elementari
\[
\Omega=\{NN,ND,DN,DD\}
\]
siano equiprobabili.

Sia
\[
A=\{DD\}
\]
l'evento in cui entrambe le esposizioni si deteriorano, e sia
\[
B=\{ND,DN,DD\}
\]
l'evento in cui almeno una delle due esposizioni si deteriora. Vogliamo
calcolare la probabilit\`{a} che entrambe le esposizioni si deteriorino
sapendo che almeno una si \`{e} deteriorata.

Per definizione,
\[
\Prob(A\mid B)=\frac{\Prob(A\cap B)}{\Prob(B)}.
\]
Poich\'{e} $A\subseteq B$, si ha $A\cap B=A$. Inoltre,
\[
\Prob(A)=\frac{1}{4},\qquad\Prob(B)=\frac{3}{4}.
\]
Pertanto
\[
\Prob(A\mid B)=\frac{1/4}{3/4}=\frac{1}{3}.
\]


Il risultato mostra che l'informazione \textquotedblleft almeno una
esposizione si \`{e} deteriorata\textquotedblright\ modifica lo spazio degli
scenari rilevanti. Non si considera pi\`{u} l'intero insieme $\Omega$, ma solo
il sottoinsieme $B$ degli scenari compatibili con l'informazione osservata.
\end{example}

\begin{example}
Un modello di "credit rating" preliminare assegna a una controparte uno score intero compreso tra
$1$ e $10$, dove valori pi\`{u} elevati indicano migliore qualit\`{a}
creditizia. In assenza di ulteriori informazioni, supponiamo che i dieci
valori siano equiprobabili:
\[
\Omega=\{1,2,\ldots,10\},\qquad\Prob(\{\omega\})=\frac{1}{10}%
\quad\text{per ogni }\omega\in\Omega.
\]


Sia
\[
A=\{10\}
\]
l'evento in cui la controparte riceve lo score massimo. Supponiamo ora di
sapere che la controparte ha superato una prima soglia di ammissibilit\`{a},
cio\`{e} che lo score \`{e} almeno pari a $5$. L'evento informativo \`{e}
\[
B=\{5,6,7,8,9,10\}.
\]
Vogliamo calcolare
\[
\Prob(A\mid B).
\]


Poich\'{e} $A\subseteq B$, si ha $A\cap B=A$. Inoltre,
\[
\Prob(A)=\frac{1}{10},\qquad\Prob(B)=\frac{6}{10}.
\]
Quindi
\[
\Prob(A\mid B)=\frac{\Prob(A\cap B)}{\Prob(B)}=\frac
{1/10}{6/10}=\frac{1}{6}.
\]


L'informazione che lo score \`{e} almeno pari a $5$ elimina gli scenari
$\{1,2,3,4\}$ dallo spazio rilevante. La probabilit\`{a} condizionata dello
score massimo non \`{e} quindi $1/10$, ma $1/6$, perch\'{e} il calcolo viene
effettuato all'interno del sottoinsieme degli scenari compatibili con
l'informazione disponibile.
\end{example}

I due esempi precedenti mostrano che il condizionamento non modifica gli
eventi in s\`{e}, ma modifica lo spazio informativo rispetto al quale le loro
probabilit\`{a} vengono valutate. In entrambi i casi, l'evento condizionante
seleziona un sottoinsieme di scenari compatibili con l'informazione osservata;
la probabilit\`{a} condizionata \`{e} quindi una probabilit\`{a} relativa a
tale sottoinsieme.

\paragraph*{Regole della somma e del prodotto.}
Le operazioni fondamentali tra eventi sono l'unione e l'intersezione. 
La prima conduce alla \textbf{regola della somma} (gi\`{a} incontrata in precedenza e che qui richiamiamo brevemente), la seconda alla regola del prodotto.

Consideriamo inizialmente due eventi $A$ e $B$. 
La regola della somma riguarda la probabilit\`{a} dell'unione $A\cup B$, cio\`{e} dell'evento in cui si verifica almeno uno tra $A$ e $B$.

Se $A$ e $B$ sono disgiunti, cio\`{e}
\[
A\cap B=\emptyset,
\]
allora i due eventi non possono verificarsi simultaneamente. In questo caso
\[
\Prob(A\cup B)=\Prob(A)+\Prob(B).
\]
Se invece $A$ e $B$ non sono disgiunti, allora l'intersezione $A\cap B$ contiene gli esiti comuni ai due eventi. Tali esiti verrebbero conteggiati due volte nella somma $\Prob(A)+\Prob(B)$: una volta come parte di $A$ e una volta come parte di $B$. Per correggere questo doppio conteggio si sottrae la probabilit\`{a} dell'intersezione:
\[
\Prob(A\cup B)
=
\Prob(A)+\Prob(B)-\Prob(A\cap B).
\]

La \textbf{regola del prodotto} riguarda invece la probabilit\`{a} dell'intersezione $A\cap B$, cio\`{e} dell'evento in cui $A$ e $B$ si verificano simultaneamente. 
Nel caso generale, se $\Prob(A)>0$, si ha
\[
\Prob(A\cap B)
=
\Prob(A)\Prob(B\mid A).
\]
Equivalentemente, se $\Prob(B)>0$,
\[
\Prob(A\cap B)
=
\Prob(B)\Prob(A\mid B).
\]
Questa \`{e} la forma generale della regola del prodotto: la probabilit\`{a} che si verifichino entrambi gli eventi \`{e} data dalla probabilit\`{a} del primo evento moltiplicata per la probabilit\`{a} del secondo evento condizionata al verificarsi del primo.

Un caso particolare importante si ha quando il verificarsi di $A$ non modifica la probabilit\`{a} di $B$. In tal caso
\[
\Prob(B\mid A)=\Prob(B),
\]
e la regola del prodotto si riduce a
\[
\Prob(A\cap B)
=
\Prob(A)\Prob(B).
\]

\section{Indipendenza tra eventi}

\begin{definition}
Due eventi $A,B\in\F$ si dicono indipendenti se
\[
\Prob(A\cap B)=\Prob(A)\Prob(B).
\]

\end{definition}

Se $\Prob(B)>0$, l'indipendenza \`{e} equivalente a
\[
\Prob(A\mid B)=\Prob(A).
\]
Quindi $A$ e $B$ sono indipendenti quando la conoscenza del verificarsi di $B$
non modifica la probabilit\`{a} attribuita ad $A$.

\`{e} importante distinguere indipendenza e incompatibilit\`{a}. Se $A$ e $B$
sono disgiunti e hanno probabilit\`{a} positiva, allora non sono indipendenti:
infatti
\[
\Prob(A\cap B)=0,
\]
mentre
\[
\Prob(A)\Prob(B)>0.
\]
In finanza, l'ipotesi di indipendenza deve essere utilizzata con cautela.
Eventi di default apparentemente separati possono diventare fortemente
dipendenti in presenza di uno shock macroeconomico comune.

\section{Formula delle probabilit\`{a} totali}

Sia $B_{1},\ldots,B_{n}$ una partizione di $\Omega$ con $\Prob(B_{i})>0$
per ogni $i$. Per ogni evento $A\in\F$ vale
\[
\Prob(A)=\sum_{i=1}^{n}\Prob(A\mid B_{i})\Prob(B_{i}).
\]


La formula delle probabilit\`{a} totali consente di calcolare la
probabilit\`{a} di un evento aggregando le probabilit\`{a} condizionate
rispetto a scenari alternativi.

\begin{example}
Supponiamo che l'economia possa trovarsi in tre stati alternativi:
\[
S_1=\text{``recessione''}, \qquad
S_2=\text{``crescita moderata''}, \qquad
S_3=\text{``espansione''}.
\]
Assumiamo che questi tre eventi costituiscano una partizione dello spazio degli esiti $\Omega$. 
Ci\`{o} significa che gli stati sono mutuamente esclusivi,
\[
S_i\cap S_j=\emptyset \qquad \text{per } i\neq j,
\]
ed esaustivi,
\[
S_1\cup S_2\cup S_3=\Omega.
\]

Sia ora $A$ l'evento
\[
A=\text{``il rendimento di un portafoglio \`{e} positivo''}.
\]
La probabilit\`{a} di $A$ pu\`{o} essere calcolata considerando separatamente i tre possibili stati dell'economia. 
Infatti, l'evento $A$ pu\`{o} essere scomposto come
\[
A=(A\cap S_1)\cup(A\cap S_2)\cup(A\cap S_3).
\]
Poich\'{e} gli eventi $S_1,S_2,S_3$ sono disgiunti, anche gli eventi
\[
A\cap S_1, \qquad A\cap S_2, \qquad A\cap S_3
\]
sono disgiunti. Pertanto,
\[
\Prob(A)
=
\Prob(A\cap S_1)
+
\Prob(A\cap S_2)
+
\Prob(A\cap S_3).
\]
Applicando la regola del prodotto a ciascun termine, si ottiene
\[
\Prob(A)
=
\Prob(A\mid S_1)\Prob(S_1)
+
\Prob(A\mid S_2)\Prob(S_2)
+
\Prob(A\mid S_3)\Prob(S_3).
\]
Questa \`{e} la formula delle probabilit\`{a} totali.

Supponiamo, ad esempio, che le probabilit\`{a} dei tre stati siano
\[
\Prob(S_1)=0.20, \qquad
\Prob(S_2)=0.50, \qquad
\Prob(S_3)=0.30.
\]
Supponiamo inoltre che la probabilit\`{a} di ottenere un rendimento positivo nei tre stati sia:
\[
\Prob(A\mid S_1)=0.25, \qquad
\Prob(A\mid S_2)=0.60, \qquad
\Prob(A\mid S_3)=0.85.
\]
Allora
\[
\begin{aligned}
\Prob(A)
&=
0.25\cdot 0.20
+
0.60\cdot 0.50
+
0.85\cdot 0.30 \\
&=
0.05+0.30+0.255 \\
&=
0.605.
\end{aligned}
\]
La probabilit\`{a} complessiva che il portafoglio abbia rendimento positivo \`{e} quindi pari a $0.605$.
\end{example}

\section{Formula di Bayes}

Nel linguaggio della formula di Bayes si distingue tra probabilit\`{a} a priori e
probabilit\`{a} a posteriori. Le probabilit\`{a} a priori descrivono la
valutazione iniziale della probabilit\`{a} degli scenari, prima di osservare una
nuova informazione. Se $B_1,\ldots,B_n$ sono scenari alternativi e mutuamente
esclusivi, le quantit\`{a}
\[
\Prob(B_1),\ldots,\Prob(B_n)
\]
rappresentano le probabilit\`{a} a priori degli scenari.

Le probabilit\`{a} a posteriori descrivono invece la valutazione aggiornata
della probabilit\`{a} degli stessi scenari dopo avere osservato un evento $A$.
Esse sono quindi probabilit\`{a} condizionate del tipo
\[
\Prob(B_j\mid A),
\qquad j=1,\ldots,n.
\]
La formula di Bayes fornisce precisamente la regola con cui passare dalle
probabilit\`{a} a priori $\Prob(B_j)$ alle probabilit\`{a} a posteriori
$\Prob(B_j\mid A)$, tenendo conto della legge delle probabilit\`{a} totali (i.e. le probabilit\`{a} con cui l'informazione osservata $A$ si manifesta nei diversi scenari).
\[
\Prob(B_{j}\mid A) = \frac{\Prob(A\mid B_{j})\Prob(B_{j})}
{\sum_{i=1}^{n}\Prob(A\mid B_{i})\Prob(B_{i})}.
\]

Nel linguaggio del rischio di credito, $B_{j}$ pu\`{o} rappresentare uno stato
latente del debitore e $A$ un segnale osservabile, come un peggioramento dello
spread. La formula di Bayes consente allora di calcolare la probabilit\`{a}
aggiornata dello stato del debitore dopo il segnale.

\begin{example}

Supponiamo di voler valutare la probabilit\`{a} che l'economia entri in
recessione entro il prossimo anno. Indichiamo con
\[
R
\]
l'evento ``si verifica una recessione'' e con $R^c$ l'evento complementare
``non si verifica una recessione''. Prima di osservare nuove informazioni,
l'analista attribuisce alla recessione la probabilit\`{a}
\[
\Prob(R)=0.20,
\qquad
\Prob(R^c)=0.80.
\]

Consideriamo ora un indicatore macroeconomico, ad esempio un indice anticipatore
dell'attivit\`{a} economica. Sia
\[
S
\]
l'evento ``l'indicatore fornisce un segnale negativo''. Supponiamo che il
segnale negativo sia pi\`{u} probabile in caso di recessione che in caso di
assenza di recessione:
\[
\Prob(S\mid R)=0.75,
\qquad
\Prob(S\mid R^c)=0.25.
\]
La prima probabilit\`{a} indica che, se l'economia entrer\`{a} in recessione,
l'indicatore fornisce un segnale negativo nel $75\%$ dei casi. La seconda indica
che, anche in assenza di recessione, l'indicatore pu\`{o} comunque fornire un
segnale negativo nel $25\%$ dei casi.

Dopo avere osservato il segnale $S$, vogliamo aggiornare la probabilit\`{a} di
recessione, cio\`{e} calcolare
\[
\Prob(R\mid S).
\]
Per la formula di Bayes,
\[
\Prob(R\mid S)
=
\frac{\Prob(S\mid R)\Prob(R)}
{\Prob(S)}.
\]
Il denominatore si calcola con la formula delle probabilit\`{a} totali:
\[
\Prob(S)
=
\Prob(S\mid R)\Prob(R)
+
\Prob(S\mid R^c)\Prob(R^c).
\]
Pertanto
\[
\Prob(S)
=
0.75\cdot 0.20
+
0.25\cdot 0.80
=
0.15+0.20
=
0.35.
\]
Applicando Bayes,
\[
\Prob(R\mid S)
=
\frac{0.75\cdot 0.20}{0.35}
=
\frac{0.15}{0.35}
\simeq 0.4286.
\]

L'osservazione del segnale negativo modifica quindi la probabilit\`{a} di
recessione da $0.20$ a circa $0.429$. In altri termini, il segnale negativo non
implica con certezza una recessione, ma aumenta in modo significativo la
probabilit\`{a} attribuita a tale scenario.
    
\end{example}

\section{Esercizi svolti}

\begin{exercise}[Eventi, probabilit\`{a} condizionata e indipendenza]
\label{ex:cap1-eventi-condizionamento-indipendenza}
Un portafoglio pu\`{o} trovarsi, alla fine dell'orizzonte considerato, in uno
dei seguenti cinque scenari:
\[
\Omega=\{\omega_1,\omega_2,\omega_3,\omega_4,\omega_5\}.
\]
Le probabilit\`{a} degli scenari sono
\[
\begin{aligned}
\Prob(\{\omega_1\})&=0.10, &
\Prob(\{\omega_2\})&=0.20, &
\Prob(\{\omega_3\})&=0.25,\\
\Prob(\{\omega_4\})&=0.30, &
\Prob(\{\omega_5\})&=0.15.
\end{aligned}
\]
Si considerino gli eventi
\[
A=\{\omega_1,\omega_2,\omega_3\},
\qquad
B=\{\omega_3,\omega_4,\omega_5\},
\qquad
C=\{\omega_1,\omega_5\}.
\]
Calcolare:
\[
\Prob(A),\quad \Prob(B),\quad
\Prob(A\cap B),\quad \Prob(A\cup B),
\quad \Prob(A\mid B).
\]
Verificare inoltre se $A$ e $B$ sono indipendenti e se $A$ e $C$ sono disgiunti.
\end{exercise}

\noindent
\textbf{Soluzione.}
Si ha
\[
\Prob(A)=0.10+0.20+0.25=0.55,
\]
e
\[
\Prob(B)=0.25+0.30+0.15=0.70.
\]
L'intersezione tra $A$ e $B$ \`{e}
\[
A\cap B=\{\omega_3\},
\]
quindi
\[
\Prob(A\cap B)=0.25.
\]
La probabilit\`{a} dell'unione si calcola con la regola della somma:
\[
\Prob(A\cup B)
=
\Prob(A)+\Prob(B)-\Prob(A\cap B)
=
0.55+0.70-0.25=1.
\]
La probabilit\`{a} condizionata di $A$ dato $B$ \`{e}
\[
\Prob(A\mid B)
=
\frac{\Prob(A\cap B)}{\Prob(B)}
=
\frac{0.25}{0.70}
\simeq 0.3571.
\]

Per verificare l'indipendenza tra $A$ e $B$, confrontiamo
\[
\Prob(A\cap B)
\]
con
\[
\Prob(A)\Prob(B).
\]
Poich\'{e}
\[
\Prob(A)\Prob(B)=0.55\cdot0.70=0.385,
\]
mentre
\[
\Prob(A\cap B)=0.25,
\]
gli eventi $A$ e $B$ non sono indipendenti.

Infine,
\[
A\cap C=\{\omega_1\},
\]
quindi $A$ e $C$ non sono disgiunti.

\begin{exercise}[Aggiornamento bayesiano del rischio di default]
\label{ex:cap1-bayes-rischio-default}
Un debitore pu\`{o} trovarsi in uno di tre stati latenti:
\[
B_1=\text{solido},\qquad B_2=\text{fragile},\qquad B_3=\text{critico}.
\]
Gli stati formano una partizione di $\Omega$ e hanno probabilit\`{a} a priori
\[
\Prob(B_1)=0.50,\qquad
\Prob(B_2)=0.35,\qquad
\Prob(B_3)=0.15.
\]
Si osserva un segnale negativo $S$. Le probabilit\`{a} del segnale nei tre stati sono
\[
\Prob(S\mid B_1)=0.05,\qquad
\Prob(S\mid B_2)=0.30,\qquad
\Prob(S\mid B_3)=0.75.
\]
Le probabilit\`{a} di default condizionate allo stato sono
\[
\Prob(D\mid B_1)=0.01,\qquad
\Prob(D\mid B_2)=0.06,\qquad
\Prob(D\mid B_3)=0.25.
\]
Assumendo che, condizionatamente allo stato $B_j$, il segnale $S$ non aggiunga
ulteriore informazione diretta sul default, calcolare:
\[
\Prob(S),
\qquad
\Prob(B_j\mid S),\quad j=1,2,3,
\qquad
\Prob(D\mid S).
\]
\end{exercise}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{../graphics/Cap01_Eserc_a.png}
\caption{Diagramma degli eventi considerati nell'esercizio.}
\label{fig:eserc-cap01-a}
\end{figure}

\noindent
\textbf{Soluzione.}
La probabilit\`{a} non condizionata del segnale si ottiene con la formula delle
probabilit\`{a} totali:
\[
\begin{aligned}
\Prob(S)
&=
\Prob(S\mid B_1)\Prob(B_1)
+
\Prob(S\mid B_2)\Prob(B_2)
+
\Prob(S\mid B_3)\Prob(B_3)\\
&=
0.05\cdot0.50+0.30\cdot0.35+0.75\cdot0.15\\
&=
0.025+0.105+0.1125\\
&=
0.2425.
\end{aligned}
\]

Applicando la formula di Bayes, si ottengono le probabilit\`{a} a posteriori:
\[
\Prob(B_1\mid S)
=
\frac{0.05\cdot0.50}{0.2425}
\simeq 0.1031,
\]
\[
\Prob(B_2\mid S)
=
\frac{0.30\cdot0.35}{0.2425}
\simeq 0.4330,
\]
\[
\Prob(B_3\mid S)
=
\frac{0.75\cdot0.15}{0.2425}
\simeq 0.4639.
\]

Il segnale negativo aumenta quindi sensibilmente il peso dello stato critico,
che passa da una probabilit\`{a} a priori pari a $0.15$ a una probabilit\`{a}
a posteriori pari a circa $0.4639$.

Per calcolare la probabilit\`{a} di default dopo il segnale, ricordiamo l'ipotesi che allo stato $B_j$, il segnale $S$ non aggiunga
ulteriore informazione diretta sul default, cio\`{e} che
\[
\Prob(D\mid B_j)=\Prob(D\mid B_j \cap S)
\]
Usiamo quindi la
scomposizione rispetto agli stati:
\[
\Prob(D\mid S)
=
\sum_{j=1}^3
\Prob(D\mid B_j)\Prob(B_j\mid S).
\]
Questa formula utilizza l'ipotesi secondo cui, una volta noto lo stato $B_j$,
il segnale $S$ non modifica ulteriormente la probabilit\`{a} di default.

Pertanto
\[
\begin{aligned}
\Prob(D\mid S)
&=
0.01\cdot0.1031
+
0.06\cdot0.4330
+
0.25\cdot0.4639\\
&\simeq
0.0010+0.0260+0.1160\\
&\simeq 0.1430.
\end{aligned}
\]
Dopo l'osservazione del segnale negativo, la probabilit\`{a} aggiornata di
default \`{e} quindi circa pari al $14.30\%$.

\section{Esercizi proposti}

\begin{exercise}[Sigma-algebra generata da una partizione]
\label{ex:cap1-prop-sigma-partizione}
Sia
\[
\Omega=\{\omega_1,\omega_2,\omega_3,\omega_4,\omega_5,\omega_6\}.
\]
Si consideri la partizione
\[
\mathcal{A}
=
\big\{
A_1,A_2,A_3
\big\},
\]
dove
\[
A_1=\{\omega_1,\omega_2\},\qquad
A_2=\{\omega_3\},\qquad
A_3=\{\omega_4,\omega_5,\omega_6\}.
\]
Costruire la $\sigma$-algebra generata da $\mathcal{A}$.

Si consideri poi la partizione pi\`{u} fine
\[
\mathcal{R}
=
\big\{
R_1,R_2,R_3,R_4
\big\},
\]
dove
\[
R_1=\{\omega_1\},\qquad
R_2=\{\omega_2\},\qquad
R_3=\{\omega_3\},\qquad
R_4=\{\omega_4,\omega_5,\omega_6\}.
\]
Verificare che $\mathcal{R}$ \`{e} un raffinamento di $\mathcal{A}$ e che
\[
\sigma(\mathcal{A})\subseteq\sigma(\mathcal{R}).
\]
\end{exercise}

\begin{exercise}[Verifica dell'indipendenza]
\label{ex:cap1-prop-indipendenza}
Siano $A$ e $B$ due eventi con
\[
\Prob(A)=0.40,\qquad\Prob(B)=0.50,\qquad\Prob(A\cap B)=0.20.
\]
Verificare se $A$ e $B$ sono indipendenti.
\end{exercise}

\begin{exercise}[Aggiornamento bayesiano di uno stato fragile]
\label{ex:cap1-prop-bayes-stato-fragile}
Un debitore pu\`{o} trovarsi in due stati, solido e fragile. Le
probabilit\`{a} a priori sono $0.70$ e $0.30$. La probabilit\`{a} di osservare
un segnale negativo \`{e} $0.10$ nello stato solido e $0.50$ nello stato
fragile. Calcolare la probabilit\`{a} a posteriori dello stato fragile dopo
l'osservazione del segnale.
\end{exercise}

\section{Sintesi finale}

Uno spazio di probabilit\`{a} $(\Omega,\F,\Prob)$ fornisce la
rappresentazione formale dell'incertezza. Lo spazio campionario $\Omega$
descrive gli esiti possibili; la sigma-algebra $\F$ descrive gli
eventi considerati dal modello; la misura $\Prob$ assegna probabilit\`{a}
agli eventi.

Le operazioni sugli eventi consentono di rappresentare eventi alternativi,
congiunti o complementari. La probabilit\`{a} condizionata formalizza
l'effetto dell'informazione sulla valutazione probabilistica. L'indipendenza
rappresenta il caso in cui tale informazione non modifica la probabilit\`{a}
dell'evento di interesse.

La formula delle probabilit\`{a} totali permette di decomporre una
probabilit\`{a} rispetto a una partizione di scenari. La formula di Bayes
permette di aggiornare le probabilit\`{a} degli scenari dopo l'osservazione di
un segnale. Questi strumenti costituiscono la base per la trattazione delle
variabili casuali, dei valori attesi condizionati e dei processi stocastici
nei capitoli successivi.

