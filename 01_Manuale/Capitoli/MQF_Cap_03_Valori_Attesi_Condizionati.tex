...
La partizione non \`{e} per\`{o} l'oggetto rispetto al quale si condiziona in
senso formale. L'oggetto formale \`{e} la sigma-algebra generata dalla
...
Gli eventi di $\mathcal{G}$ sono le unioni e i complementi dei blocchi della partizione. I
blocchi $B_1,\ldots,B_n$ sono gli atomi della sigma-algebra $\mathcal{G}$:
l'informazione $\mathcal{G}$ distingue tra blocchi diversi, ma non distingue
gli esiti elementari appartenenti allo stesso blocco.
...
Questa quantit\`{a} \`{e} un numero. Il valore atteso condizionato di $X$
rispetto alla sigma-algebra $\mathcal{G}$ \`{e} invece la variabile casuale:
\[
...
Questa formula va letta sugli atomi della sigma-algebra. Se $\omega\in B_i$,
allora:
\[
...
Pertanto $\mathbb{E}[X\mid\mathcal{G}]$ \`{e} una variabile casuale costante
su ciascun blocco della partizione. Essa assume al pi\`{u} $n$ valori
distinti, uno per ciascun atomo $B_i$ di $\mathcal{G}$. Non assume, in
generale, tanti valori quanti sono gli eventi non vuoti di $\mathcal{G}$.

Gli eventi non vuoti di $\mathcal{G}$ sono invece rilevanti per un'altra
ragione: per ciascun evento osservabile $C\in\mathcal{G}$, con
$\mathbb{P}(C)>0$, si pu\`{o} calcolare il numero $\mathbb{E}[X\mid C]$.
Questi valori descrivono le medie condizionate rispetto agli eventi
osservabili, mentre $\mathbb{E}[X\mid\mathcal{G}]$ \`{e} una variabile casuale
definita sugli esiti elementari.

\medskip

\noindent
\textbf{Esempio con sette scenari elementari.}
...
Sia $H$ un payoff terminale definito sui sette scenari. Supponiamo che
probabilit\`{a} e payoff siano dati dalla tabella seguente:
\[
\begin{array}{c|c|c}
\text{Scenario} & \mathbb{P}(\omega) & H(\omega) \\
\hline
...
L'informazione disponibile non distingue tutti gli scenari elementari, ma li
raggruppa in tre blocchi informativi:
\[
B_1=\{\omega_1,\omega_2\},
\qquad
...
I tre blocchi hanno probabilit\`{a}:
\[
\mathbb{P}(B_1)=0.25,
\qquad
\mathbb{P}(B_2)=0.45,
...
Calcoliamo ora i valori attesi condizionati rispetto ai tre atomi della
...
Quindi il valore atteso condizionato rispetto alla sigma-algebra
$\mathcal{G}$ \`{e}:
\[
...
Questa variabile casuale assume tre valori, uno per ciascun atomo della
sigma-algebra:
\[
\begin{array}{c|c|c|c}
...
Questa tabella mostra la distinzione essenziale: $H$ distingue i sette
scenari elementari, mentre $\mathbb{E}[H\mid\mathcal{G}]$ distingue soltanto i
tre blocchi informativi.

\medskip

\noindent
\textbf{Eventi osservabili e medie condizionate.}

La sigma-algebra $\mathcal{G}$ contiene sette eventi non vuoti. Per ciascuno
di essi \`{e} possibile calcolare un valore atteso condizionato rispetto
all'evento. Questi sette numeri non sono per\`{o} i valori puntuali della
variabile casuale $\mathbb{E}[H\mid\mathcal{G}]$.

Essi sono:
\[
\begin{array}{c|c}
\text{Evento osservabile } C & \mathbb{E}[H\mid C] \\
\hline
B_1 & 118 \\
B_2 & 91.111 \\
B_3 & 62 \\
...
Questa tabella ha una diversa interpretazione. Essa associa a ciascun evento
osservabile $C\in\mathcal{G}$, con $C\neq\varnothing$, il numero
$\mathbb{E}[H\mid C]$. Invece la variabile casuale
$\mathbb{E}[H\mid\mathcal{G}]$ associa a ciascuno scenario elementare
$\omega$ il valore medio del blocco informativo a cui $\omega$ appartiene.

La relazione tra le due rappresentazioni \`{e} la seguente. Se
$C\in\mathcal{G}$ \`{e} unione di alcuni blocchi della partizione, allora:
\[
\mathbb{E}[H\mid C]
=
\frac{
\mathbb{E}\left[
\mathbb{E}[H\mid\mathcal{G}]\mathbf{1}_C
\right]
}
{\mathbb{P}(C)}.
\]

Quindi le medie condizionate rispetto agli eventi osservabili si ottengono
mediando la variabile casuale $\mathbb{E}[H\mid\mathcal{G}]$ sull'evento
osservabile considerato.
...
La costruzione precedente suggerisce la definizione generale. Sia
$\mathcal{G}\subseteq\mathcal{F}$ una sigma-algebra informativa e sia $X$ una
variabile casuale integrabile. Un valore atteso condizionato di $X$ rispetto a
$\mathcal{G}$ \`{e} una variabile casuale $\mathbb{E}[X\mid\mathcal{G}]$ tale
che:
...
meno di uguaglianza quasi certa. Due versioni di
$\mathbb{E}[X\mid\mathcal{G}]$ sono quindi considerate equivalenti se
coincidono tranne che su un evento di probabilit\`{a} nulla.