...
La Sezione~\ref{sec:cap15-motivazione-multistadio-v2} ha introdotto
la successione delle decisioni di ricorso $y_k^s$ e la condizione
di non anticipativit\`a
\eqref{eq:cap15-non-anticipativita-multistadio-v2}.
Possiamo ora formulare il problema di ottimizzazione che comprende
la decisione iniziale $x$ e le decisioni assunte nei diversi stadi.

Il passaggio dal modello a due stadi al multistadio richiede di
specificare quali evoluzioni future rimangano possibili dopo
ciascuna storia osservata. Su questa struttura informativa
vengono poi definiti la funzione obiettivo e i vincoli che
collegano le decisioni successive.


\subsection{La struttura degli scenari}

Consideriamo un insieme finito $\mathcal S$ di scenari completi.
Lo scenario $s$ identifica una possibile traiettoria
dell'incertezza sull'intero orizzonte ed \`e associato
a una probabilit\`a $p_s$, con
...
Nel modello a due stadi del Capitolo~
\ref{cap:programmazione-stocastica-due-stadi}, il ricorso
viene scelto dopo l'osservazione dello scenario completo.
Nel modello multistadio, invece, dopo uno stadio intermedio
possono rimanere possibili pi\`u prosecuzioni della storia
osservata.

Per rappresentare questa struttura utilizziamo un
\emph{albero degli scenari}. La radice corrisponde
all'informazione disponibile al tempo $0$, prima
della scelta di $x$. Ciascun nodo di uno stadio
successivo identifica una storia osservata fino
al termine di quello stadio.

Un nodo $n$ pu\`o avere pi\`u figli, corrispondenti
a differenti prosecuzioni della medesima storia.
Ogni nodo diverso dalla radice ha un unico predecessore,
indicato con $a(n)$. Un percorso dalla radice a un
nodo terminale identifica uno scenario completo.

La Figura~\ref{fig:cap15-albero-generico-v2}
illustra questa costruzione.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.70\textwidth]
{../graphics/Cap15_albero_multistadio_generico.png}
\caption{Rappresentazione ad albero degli scenari.
La radice corrisponde alla decisione iniziale;
i nodi successivi identificano le storie informative
osservate. Da un nodo possono svilupparsi differenti
prosecuzioni della medesima storia. Un percorso dalla
radice a un nodo terminale rappresenta uno scenario
completo.}
\label{fig:cap15-albero-generico-v2}
\end{figure}

A ciascun nodo $n$ dello stadio $k$ pu\`o essere
associata una sola decisione di ricorso $y_n$.
Nella rappresentazione per scenari completi,
la stessa decisione viene invece indicata con
$y_k^s$ per tutti gli scenari $s$ che attraversano
quel nodo. Le due scritture corrispondono alla
medesima struttura decisionale; nella seconda,
la coincidenza delle decisioni \`e imposta dalla
\eqref{eq:cap15-non-anticipativita-multistadio-v2}.

Nel seguito utilizziamo la rappresentazione
per scenari completi, che permette di mantenere
la continuit\`a con il Capitolo 14. La struttura
ad albero rimane sottostante alla formulazione
anche quando i nodi non compaiono esplicitamente
fra gli indici delle variabili.

\medskip

\noindent
\textbf{Scenari simulati e struttura informativa.}

Le traiettorie di un processo stocastico possono
essere generate mediante simulazione Monte Carlo.
Tuttavia, un campione di traiettorie complete
generate indipendentemente non costituisce,
di per s\'e, una rappresentazione adeguata
del ricorso multistadio.

Consideriamo, ad esempio, un campione di traiettorie
di un processo a valori continui. Se le storie
osservate al termine del primo stadio sono tutte
differenti, nel campione finito ciascuna di esse
identifica gi\`a uno scenario completo. A quella
storia corrisponde quindi una sola prosecuzione
futura nel modello campionario.

Il problema risultante conserva pi\`u blocchi
decisionali e pi\`u vincoli temporali, ma non
rappresenta un'effettiva incertezza sulle
prosecuzioni successive alla prima osservazione.
Sotto il profilo informativo, il ricorso pu\`o
essere determinato conoscendo l'intera traiettoria,
come in un problema a due stadi.

Per costruire un'approssimazione finita
multistadio mediante scenari \`e pertanto
necessario conservare le alternative future
dopo le storie intermedie. La simulazione
pu\`o essere utilizzata a questo scopo
generando, a partire da una storia comune,
differenti prosecuzioni nei successivi
stadi decisionali.

L'albero non richiede che ogni nodo abbia
lo stesso numero di figli, n\'e che tutti
gli scenari completi abbiano la stessa
probabilit\`a. Nella formulazione che segue
i pesi $p_s$ sono le probabilit\`a degli
scenari completi, indipendentemente dalla
particolare procedura utilizzata per
costruire l'albero.


\subsection{Il programma lineare multistadio}

La struttura degli scenari definisce quali informazioni sono
disponibili quando vengono assunte le decisioni. Occorre ora
specificare il problema di ottimizzazione: il criterio economico
da massimizzare e i vincoli che collegano la decisione iniziale
alle decisioni dei diversi stadi.

Concentriamo l'analisi sui programmi stocastici lineari.
La linearit\`a non \`e una propriet\`a necessaria della
programmazione stocastica, ma individua una classe di modelli
particolarmente rilevante per il corso, sia sul piano
matematico sia su quello applicativo.

Dal punto di vista matematico, quando la funzione obiettivo
e i vincoli sono lineari, un modello multistadio con un
numero finito di scenari pu\`o essere ricondotto a un
programma lineare deterministico. Le decisioni dei diversi
stadi e scenari diventano variabili di un unico problema;
anche i vincoli di non anticipativit\`a sono uguaglianze
lineari. Possiamo quindi utilizzare i metodi risolutivi
della programmazione lineare gi\`a studiati nel corso,
senza introdurre una nuova classe di algoritmi di
ottimizzazione.

La formulazione lineare rende inoltre esplicita la
struttura dei collegamenti tra stadi. Questa struttura
pu\`o essere sfruttata dai metodi di decomposizione
quando il numero degli scenari rende onerosa la
soluzione diretta del problema complessivo.

Dal punto di vista applicativo, numerosi problemi
finanziari comprendono relazioni naturalmente lineari:
i vincoli di bilancio, la somma dei flussi di cassa,
la ripartizione di risorse disponibili, i limiti
di investimento e le condizioni di equilibrio
tra fonti e impieghi. I coefficienti di tali
relazioni possono cambiare con lo scenario senza
che venga meno la linearit\`a rispetto alle
variabili decisionali.

Il caso lineare permette pertanto di studiare
congiuntamente tre aspetti essenziali:
la scelta iniziale sotto incertezza, l'adattamento
progressivo delle decisioni e il rispetto dei
vincoli finanziari lungo l'orizzonte. Eventuali
relazioni economiche non lineari richiedono
invece modelli o approssimazioni ulteriori,
che non sono necessari per sviluppare
i principi del programma multistadio qui considerato.
...
Manteniamo la convenzione di massimizzazione adottata
nel Capitolo~\ref{cap:programmazione-stocastica-due-stadi}.
Indichiamo con $c$ il vettore dei coefficienti economici
della decisione iniziale $x$ e con $q_k^s$ il vettore
dei coefficienti economici del blocco decisionale
$y_k^s$ nello scenario $s$.

In una formulazione lineare, il contributo economico
delle decisioni nello scenario $s$ \`e rappresentato da

\[
c'x+\sum_{k=1}^{K}(q_k^s)'y_k^s.
\]

La decisione iniziale contribuisce una sola volta
al risultato economico. Le decisioni successive
contribuiscono attraverso i rispettivi blocchi,
con coefficienti che possono essere differenti
nei diversi stadi e scenari.

Ponderando il risultato di ciascuno scenario
con la probabilit\`a $p_s$, otteniamo la
funzione obiettivo
...
\sum_{k=1}^{K}(q_k^s)'y_k^s
\right\}.
\label{eq:cap15-obiettivo-lineare-multistadio-v2}
\end{equation}

L'espressione
\eqref{eq:cap15-obiettivo-lineare-multistadio-v2}
specifica il criterio economico, ma non costituisce
ancora il programma multistadio completo. Le decisioni
devono soddisfare i vincoli iniziali, i vincoli di
collegamento tra stadi e la condizione informativa
\eqref{eq:cap15-non-anticipativita-multistadio-v2}.

\medskip

\noindent
\textbf{Vincoli iniziali e vincoli di collegamento.}

L'insieme $\mathcal X$ contiene i vincoli
iniziali, formulati con dati noti al tempo
$0$. Tali vincoli restano distinti dai
vincoli relativi agli stadi successivi.

Per questi ultimi adottiamo la forma
matriciale di uguaglianza

\begin{equation}
T_{s,k}x+
\sum_{j=1}^{k}W_{s,k,j}y_j^s
=
h_{s,k},
\qquad
k=1,\ldots,K,\quad s\in\mathcal S.
\label{eq:cap15-vincoli-stadio-v2}
\end{equation}

La matrice $T_{s,k}$ descrive il contributo
della decisione iniziale $x$ ai vincoli
dello stadio $k$ nello scenario $s$.
Non \`e la matrice dei vincoli imposti
alla decisione iniziale al tempo $0$.

La matrice $W_{s,k,j}$ descrive il
contributo della decisione assunta
nello stadio $j$ ai vincoli dello
stadio $k$. Il vettore $h_{s,k}$
raccoglie i termini noti di tali vincoli.

Il pedice $s$ identifica lo scenario;
$k$ identifica lo stadio al quale
appartengono le equazioni e $j$
identifica lo stadio della decisione
che compare nelle equazioni.

La somma nella
\eqref{eq:cap15-vincoli-stadio-v2}
comprende soltanto le decisioni
assunte fino allo stadio corrente.
I vincoli dello stadio $k$ non
dipendono dalle decisioni degli
stadi successivi.

\medskip

\noindent
\textbf{Dimensioni delle matrici.}

Indichiamo con $n_x$ il numero delle
componenti della decisione iniziale,
con $d_j$ il numero delle componenti
del blocco decisionale $y_j^s$
e con $m_k$ il numero delle equazioni
di collegamento dello stadio $k$.

Assumendo che queste dimensioni
non dipendano dallo scenario, abbiamo
...
y_j^s&\in\R^{d_j},
\\[3pt]
T_{s,k}&\in\R^{m_k\times n_x},
&
W_{s,k,j}&\in\R^{m_k\times d_j},
\\[3pt]
h_{s,k}&\in\R^{m_k}.
\end{aligned}
\]

Il numero $m_k$ conta le equazioni
di collegamento dello stadio $k$,
mentre $d_j$ conta le variabili
decisionali del blocco $j$.
Stadi differenti possono avere
numeri diversi di variabili e
di equazioni.

Il simbolo $d_j$ \`e distinto
da $n_j$, gi\`a utilizzato
per indicare il numero di tempi
fisici appartenenti allo stadio $j$.

Le matrici $T_{s,k}$ e
$W_{s,k,j}$ e il vettore
$h_{s,k}$ possono variare
per scenario e per stadio.
I loro valori, quando compaiono
nei vincoli attribuiti allo
stadio $k$, sono determinati
dalla storia informativa
disponibile entro $\bar t_k$.
...
Indichiamo con $\mathcal X_{s,k}$
l'insieme degli eventuali vincoli
operativi propri del blocco
$y_k^s$, quali limiti inferiori
e superiori sulle sue componenti.

Il programma lineare multistadio
pu\`o essere scritto nella
seguente forma estesa:
...
si aggiungono i vincoli di non
anticipativit\`a
\eqref{eq:cap15-non-anticipativita-multistadio-v2}.

La forma estesa riunisce tutte
le decisioni e tutti gli scenari
in un unico programma lineare
deterministico. I pesi $p_s$
compaiono nella funzione obiettivo,
mentre la struttura ad albero
determina quali decisioni
scenario-specifiche devono
coincidere.

La rappresentazione mediante
uguaglianze non esclude altri
vincoli lineari: disuguaglianze
e limiti di capacit\`a possono
essere aggiunti esplicitamente
oppure ricondotti, quando
opportuno, alla forma di
uguaglianza mediante variabili
ausiliarie.

\medskip

\noindent
\textbf{Raccordo con la notazione a due stadi.}

Le matrici e i vettori dello
stadio $k$ possono essere letti
come blocchi degli oggetti
matriciali complessivi dello
scenario $s$.

Raccogliendo per righe i blocchi
$T_{s,k}$ e i vettori $h_{s,k}$
si ottengono rispettivamente
$T_s$ e $h_s$. Ordinando i
blocchi decisionali per stadio,
si ottiene una matrice complessiva
$W_s$ con struttura triangolare
inferiore a blocchi: i blocchi
sopra la diagonale sono nulli,
poich\`e i vincoli correnti
non coinvolgono decisioni future.

Questa lettura conserva la
convenzione $T_s$, $W_s$ e
$h_s$ introdotta nel
Capitolo~\ref{cap:programmazione-stocastica-due-stadi},
senza imporre che i coefficienti
dei diversi blocchi siano uguali.

Se l'intero orizzonte successivo
al tempo iniziale costituisce
un unico stadio di ricorso,
si pone $K=1$. Identificando
...
la formulazione multistadio
si riconduce alla forma estesa
a due stadi
\eqref{eq:cap14-forma-estesa}.

La sezione successiva specializza
la formulazione generale al caso
SVB--ALM, identificando le
decisioni, i coefficienti e
i vincoli finanziari del modello.