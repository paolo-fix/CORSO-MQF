...
\textit{Risultati:} nel primo scenario,
...
\[
\mathrm{PV}_{\mathrm{prot}}=6.804,
\qquad
s^*\approx0.02574,
\]

ossia circa $257.37$ punti base. Inoltre,

\[
s^*(R=0.30)\approx300.26
\text{ punti base},
\]

\[
s^*(R=0.50)\approx214.47
\text{ punti base}.
\]

Nel secondo scenario, con $R=0.40$,

\[
s^*\approx0.03517,
\]

ossia circa $351.65$ punti base.
...
Il capitolo ha mostrato come le probabilit\`{a} di transizione tra
rating possano essere trasformate in una distribuzione delle perdite
creditizie. Il percorso richiede di collegare la dinamica degli stati,
le conseguenze economiche delle migrazioni e le misure utilizzate per
valutare il rischio.

La sequenza concettuale pu\`{o} essere sintetizzata come segue:
...
Le potenze della matrice di transizione consentono di ricavare le
probabilit\`{a} cumulative e marginali di default e le corrispondenti
probabilit\`{a} di sopravvivenza. Queste quantit\`{a} descrivono
l'evoluzione probabilistica del rischio creditizio, ma non determinano
ancora una perdita monetaria.

Per costruire la distribuzione della perdita occorre integrare le
probabilit\`{a} di default e di migrazione con l'\emph{Exposure at
Default} (\emph{EAD}), la \emph{Loss Given Default} (\emph{LGD}) e i
valori associati alle diverse classi di rating. La funzione di perdita
trasforma cos\`{\i} ciascuno stato futuro in una conseguenza economica.

Sulla distribuzione ottenuta possono essere calcolate la
\emph{Expected Loss} (\emph{EL}), il \emph{Value at Risk}
(\emph{VaR}) e il \emph{Conditional Value at Risk} (\emph{CVaR}).
La perdita attesa misura il valore medio, il VaR identifica un quantile
della distribuzione e il CVaR considera la severit\`{a} delle perdite
nella coda.

La distinzione tra $P^{\Prob}$ e $P^{\Q}$ separa due finalit\`{a}
differenti. La misura fisica viene utilizzata per prevedere le perdite
e misurare il rischio; la misura risk-neutral viene utilizzata per il
pricing dei flussi creditizi e, in particolare, per la valutazione del
CDS.

Il CDS pu\`{o} ridurre la perdita associata al default, ma la copertura
rimane esposta ai premi contrattuali, al rischio di migrazione, al
rischio di controparte, al basis risk e alla liquidit\`{a} del mercato.

Nel passaggio al portafoglio, le distribuzioni individuali devono
essere integrate con un modello della dipendenza tra debitori. La
concentrazione, i fattori comuni e il rischio sistematico determinano
la probabilit\`{a} di perdite congiunte elevate.

La Lezione~10 tradurr\`{a} questa costruzione teorica in una procedura
computazionale. Mediante la simulazione delle transizioni dei rating,
dei default, delle esposizioni e dei recuperi verr\`{a} generata una
distribuzione empirica della perdita di portafoglio, dalla quale
stimare
...
Il raccordo tra le due lezioni pu\`{o} quindi essere espresso come

\[
\boxed{
\begin{array}{c}
\text{costruzione teorica del modello di perdita}
\\[2mm]
\downarrow
\\[2mm]
...
\downarrow
\\[2mm]
\text{stima empirica delle misure di rischio}
\end{array}
}
\]