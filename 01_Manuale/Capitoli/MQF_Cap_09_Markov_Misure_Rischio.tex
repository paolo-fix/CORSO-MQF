...
\item il \emph{nozionale} $N$, sul quale vengono calcolati sia i premi sia il
pagamento di protezione;

\item il \emph{premio periodico}, determinato mediante uno spread contrattuale
$s$;
...
Questa \`{e} la convenzione dei premi \textbf{posticipati}: il premio
del periodo $h$ \`{e} pagato alla fine del periodo, quando la
sopravvivenza a tale data \`{e} osservabile.

Nel caso limite di un contratto a un periodo con premio interamente
\textbf{upfront}, versato alla stipula, il pagamento avviene in $t=0$
ed \`{e} certo: poich\'{e} $d_0=1$ e $\Q(\tau_D>0\mid X_0=i)=1$, la
gamba dei premi si riduce a
...
Un pagamento pu\`{o} essere condizionato soltanto all'informazione
disponibile alla data in cui avviene: il premio upfront, versato
quando la sopravvivenza \`{e} certa, non incorpora alcuna
probabilit\`{a} n\'{e} alcuno sconto.
...
\paragraph{Un esempio sul caso guida.}

Applichiamo la costruzione all'emittente del caso guida
(Sezione~9.9), con un contratto a un periodo, $H=1$, e premio
interamente upfront. Il pricing richiede la riga risk-neutral dello
stato iniziale, che \`{e} assegnata, coerente con i prezzi di mercato:

\[
\begin{array}{lcccc}
\hline
& \text{stato }1 & \text{stato }2 & \text{stato }3 & D\\
\hline
...
Rispetto alla riga fisica, la riga risk-neutral sposta massa dagli
stati favorevoli verso il downgrade e il default. \`{E} la traduzione,
in termini di pesi, del fatto che il mercato, avverso al rischio,
valuta la protezione pi\`{u} di quanto farebbe un investitore
neutrale: le probabilit\`{a} di default implicite nei prezzi dei CDS
risultano sistematicamente superiori a quelle storiche.

Con $R=0.40$ e $d_1=1/1.05$, la gamba di protezione vale

\[
\mathrm{PV}_{\mathrm{prot}}
=
N(1-R)\,d_1\,\Q(\tau_D=1\mid X_0=2)
=
...
mentre la gamba dei premi upfront \`{e}
$\mathrm{PV}_{\mathrm{prem}}(s)=sN$. Il premio equo risolve
$s^{*}N=\mathrm{PV}_{\mathrm{prot}}$:
...
ossia $200$ punti base del nozionale. La
Figura~\ref{fig:cap09-cds-equilibrio-caso-guida} rappresenta
l'equilibrio: la gamba di protezione non dipende da $s$ ed \`{e} una
retta orizzontale; la gamba dei premi cresce linearmente in $s$; il
premio equo \`{e} l'ascissa dell'intersezione. Il valore
$s^{*}N=2$ \`{e} il costo dei premi che verr\`{a} utilizzato nella
costruzione della posizione coperta della Sezione~12.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]
{../graphics/Cap09_CDS_equilibrio_gambe_caso_guida.png}
\caption{Determinazione del premio equo del CDS sul caso guida, con
contratto a un periodo e premio upfront. La gamba di protezione,
indipendente da $s$, vale $2$; la gamba dei premi \`{e} $sN$. Il
premio equo $s^{*}=200$ punti base rende i due valori attuali uguali.}
\label{fig:cap09-cds-equilibrio-caso-guida}
\end{figure}
...
Dal 2009, inoltre, i contratti standard prevedono una cedola corrente
fissata a valori convenzionali; la differenza rispetto al premio equo
\`{e} regolata mediante un pagamento upfront alla stipula, di segno
positivo o negativo. Anche in questo assetto la condizione di
equilibrio conserva la stessa struttura.
...
\item richieda un premio interamente upfront, versato alla stipula,
pari al premio equo determinato nella Sezione~11:
$C^{\mathrm{premi}}=s^{*}N=2$.
