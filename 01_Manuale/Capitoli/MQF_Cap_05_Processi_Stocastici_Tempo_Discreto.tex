...
dell'intera matematica finanziaria: la \emph{martingala}. Una
martingala formalizza l'idea di \emph{gioco equo}: in ogni
istante, la migliore previsione del valore futuro del processo
coincide con il valore corrente. Questo concetto, che appare
semplice, \`{e} alla base della valutazione dei derivati e
della teoria dei mercati efficienti.
...
condizione~(iii) \`{e} la propriet\`{a} sostanziale.

\begin{remark}
\`{E} sufficiente verificare la condizione~(iii) sul passo
singolo,
\[
\E[X_{t+1}\mid\F_t]=X_t \qquad \text{per ogni } t,
\]
poich\`{e} la versione generale segue per induzione applicando
ripetutamente la propriet\`{a} della torre.
\end{remark}

Rilassando l'uguaglianza in~(iii) si ottengono due nozioni
collegate.
...
Una submartingala descrive un gioco \emph{favorevole} al
giocatore: il valore atteso futuro supera il valore presente.
Una supermartingala descrive un gioco \emph{sfavorevole}: il
valore atteso futuro \`{e} inferiore al valore presente.
Un'azione azionaria in un mercato con rendimento atteso positivo
\`{e} tipicamente una submartingala; il capitale di un giocatore
alla roulette, dove il banco detiene un vantaggio strutturale,
\`{e} una supermartingala.

Due conseguenze immediate della definizione meritano di essere
esplicitate.

\noindent\textbf{Media costante.} Prendendo il valore atteso
della condizione~(iii) e applicando la legge della torre:
\[
\E[X_t]=\E\bigl[\E[X_t\mid\F_s]\bigr]=\E[X_s].
\]
La costanza della media \`{e} necessaria ma non sufficiente
per la propriet\`{a} di martingala: la condizione~(iii) vincola
la previsione condizionata in ogni possibile stato
dell'informazione, non solo la media incondizionata.

\noindent\textbf{Incrementi con media condizionata nulla.}
Se $\{X_t\}$ \`{e} una martingala, allora per ogni $t$:
\[
\E[X_{t+1}-X_t\mid\F_t]=0.
\]
Gli incrementi futuri non sono prevedibili sulla base
dell'informazione corrente: il processo non ha tendenza
sistematica in alcuna direzione.

\begin{example}[Profitto cumulato di una strategia di trading]
\label{es:pl-martingala}
Riprendiamo la passeggiata aleatoria
dell'Esempio~\ref{es:random-walk}, interpretata come profitto
cumulato $X_t=\sum_{k=1}^{t}Z_k$ di una strategia di trading,
dove i profitti giornalieri $Z_k$ sono i.i.d.\ con media
$\nu$ e varianza $\tau^2$. Per ogni $s\leq t$,
dall'indipendenza dei profitti futuri dall'informazione passata:
\[
\E[X_t\mid\F_s]=X_s+(t-s)\,\nu.
\]
Il segno di $\nu$ determina interamente la natura del processo:
\begin{itemize}
\item $\nu=0$: martingala --- la strategia non ha vantaggio
sistematico, il profitto atteso futuro coincide con quello
gi\`{a} realizzato;
\item $\nu>0$: submartingala --- la strategia ha un alpha
positivo, il profitto atteso cresce nel tempo;
\item $\nu<0$: supermartingala --- i costi superano il
rendimento lordo, il profitto atteso decresce nel tempo.
\end{itemize}
\end{example}

\subsection{Decomposizione di Doob--Meyer}
...
=X_t-t\nu=M_t.
\]
Il processo $A_t=t\,\nu$ \`{e} deterministico e
$\F_{t-1}$-misurabile: rappresenta il drift accumulato, noto
un giorno in anticipo. L'alpha della strategia \`{e}
esattamente $\nu$ per periodo.
\end{example}
...
payoff terminale di un contratto, $H=g(X_T)$. Il
\emph{processo delle previsioni}
\[
H_t=\E[H\mid\F_t], \qquad t=0,1,\ldots,T,
\]
\`{e} una martingala rispetto a $\{\F_t\}$. La verifica
segue dalla propriet\`{a} della torre: per ogni $s\leq t$,
\[
\E[H_t\mid\F_s]
=\E\bigl[\E[H\mid\F_t]\bigm|\F_s\bigr]
=\E[H\mid\F_s]=H_s.
\]
Agli estremi: $H_0=\E[ht]$ (la sola informazione disponibile
\`{e} la media incondizionata) e $H_T=H$ (l'informazione
\`{e} completa e la previsione coincide con il valore
realizzato).

Gli incrementi del processo delle previsioni hanno media
condizionata nulla:
\[
\E[H_{t+1}-H_t\mid\F_t]=0.
\]
Le revisioni successive di una previsione razionale sono
shock imprevedibili: se fossero prevedibili, la previsione
corrente non starebbe gi\`{a} incorporando tutta
l'informazione disponibile, in contraddizione con la sua
definizione. Questo risultato ha un legame diretto con la
teoria dei mercati efficienti: in un mercato efficiente le
variazioni di prezzo non sono sistematicamente prevedibili
sulla base dell'informazione passata.
\end{example}

\subsection{Trasformata di martingala e assenza di arbitraggio}

Una questione fondamentale nella pratica finanziaria \`{e} se
sia possibile trasformare un gioco equo in uno favorevole
attraverso una strategia di trading opportunamente scelta.
La teoria delle martingale risponde in modo netto.

Sia $\{X_t\}$ una martingala e $\{\phi_t\}$ una strategia di
trading \emph{prevedibile}, cio\`{e} tale che $\phi_t$ sia
$\F_{t-1}$-misurabile: la posizione al tempo $t$ si sceglie
...
\begin{proof}
Per ogni $t\geq 1$, usando la $\F_{t-1}$-misurabilit\`{a}
di $\phi_t$ e la propriet\`{a} di martingala di $\{X_t\}$:
\begin{eqnarray*}
...
Nessuna strategia prevedibile trasforma un gioco equo in uno
favorevole: il guadagno atteso \`{e} sempre nullo, qualunque
sia la regola di trading adottata, purch\'{e} basata solo
sull'informazione passata. Questa \`{e} la formulazione
probabilistica dell'assenza di arbitraggio.

\begin{remark}[Arresto opzionale]
Il teorema di arresto opzionale \`{e} un caso particolare
di questo risultato. Sia $\tau$ un tempo di uscita
\emph{limitato}, cio\`{e} tale che $\tau\leq T$ quasi
certamente. Allora $\E[X_\tau]=\E[X_0]$: scegliere quando
smettere di giocare, sulla base della sola informazione
passata, non altera il valore atteso. Strategie di
\emph{market timing} basate su informazione storica non
creano valore atteso in un mercato privo di arbitraggio.
\end{remark}

\subsection{Misura risk-neutral e valutazione dei derivati}

La propriet\`{a} di martingala non \`{e} assoluta: \`{e}
relativa alla misura di probabilit\`{a} rispetto alla quale
si calcolano i valori attesi. Lo stesso processo pu\`{o} essere
una submartingala sotto una misura e una martingala sotto
un'altra. Questa osservazione \`{e} alla base della valutazione
dei derivati.
...
investitori richiedono un premio al rischio e le azioni
rendono in media pi\`{u} del tasso privo di rischio, quindi
$\{\tilde{S}_t\}$ \`{e} tipicamente una submartingala. Esiste
tuttavia una misura di probabilit\`{a} $\Q$, detta
\emph{misura risk-neutral}, equivalente a $\Prob$ ---
cio\`{e} con gli stessi eventi di probabilit\`{a} zero ---
sotto la quale il prezzo scontato diventa una martingala.

Il passaggio da $\Prob$ a $\Q$ redistribuisce i pesi degli
scenari: scenari sfavorevoli ricevono peso maggiore, scenari
favorevoli peso minore, fino a rendere il gioco equo. Il
collegamento tra no-arbitraggio e martingale \`{e} precisato
dai teoremi fondamentali del pricing.

\begin{theorem}[Teoremi fondamentali del pricing]
\label{thm:ftap}
Sia dato un mercato finanziario a tempo discreto con tasso
privo di rischio $r$.
\begin{enumerate}
\item \emph{Primo teorema}: il mercato \`{e} privo di
arbitraggio se e solo se esiste almeno una misura $\Q$,
equivalente a $\Prob$, sotto la quale il prezzo scontato di
ogni attivit\`{a} \`{e} una martingala.
\item \emph{Secondo teorema}: il mercato \`{e} completo ---
ogni payoff \`{e} replicabile con una strategia autofinanziante
--- se e solo se la misura risk-neutral $\Q$ \`{e} unica.
\end{enumerate}
\end{theorem}

Quando esiste $\Q$, il prezzo in $t=0$ di un derivato con
payoff terminale $H$ \`{e}:
\[
\pi_0=\E_{\Q}\!\left[\frac{H}{(1+r)^T}\right].
\]
La valutazione si riduce al calcolo di un valore atteso
scontato sotto $\Q$. \`{E} questa la formula operativa che
il Capitolo~9 svilupper\`{a} in dettaglio.

\begin{example}[Misura risk-neutral nel modello binomiale]
\label{es:misura-q-binomiale}
Nel modello binomiale a un periodo con $S_1^u=uS_0$,
$S_1^d=dS_0$ e tasso $r$, la condizione che il prezzo scontato
sia una martingala sotto $\Q$ \`{e}:
\[
...
purch\'{e} $d<1+r<u$: se il tasso privo di rischio non stesse
tra i fattori di ribasso e rialzo, esisterebbero opportunit\`{a}
di arbitraggio banali. Il prezzo di un'opzione call con strike
$K$ \`{e} allora:
\[
...
correzione per il rischio, sotto la quale tutte le attivit\`{a}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.92\textwidth]{../graphics/Cap05_Figura_5_1}
...
\begin{exercise}[Dimensionamento Monte Carlo]
\label{ex:montecarlo}
